...
class Migrator(ast.NodeTransformer):
    ...
    def visit_Call(self, node: ast.Call) -> ast.AST:
        ...
        if isinstance(func, ast.Attribute) and func.attr == "create_network":
            ...
            if len(node.args) == 0:
                ...
                flags_kw = next(
                    (kw for kw in node.keywords if kw.arg == "flags"), None)
    ...
    def _is_weak_network_arg(arg: ast.expr) -> bool:
        ...
        if isinstance(arg, ast.BinOp) and isinstance(arg.op, ast.LShift):
            ...
            if isinstance(inner, ast.Call) and isinstance(
                    inner.func, ast.Name) and inner.func.id == "int":
                if inner.args:
    ...
    def _already_strongly_typed(arg: ast.expr) -> bool:
        if isinstance(arg, ast.BinOp) and isinstance(arg.op, ast.LShift):
            ...
            if isinstance(inner, ast.Call) and isinstance(
                    inner.func, ast.Name) and inner.func.id == "int":
                if inner.args and _network_flag_name(
                        inner.args[0]) == "STRONGLY_TYPED":
                    return True
        return False
    ...
    def visit_Expr(self, node: ast.Expr) -> ast.AST | None:
        ...
        if isinstance(node.value, ast.Call):
            ...
            if isinstance(call.func,
                          ast.Attribute) and call.func.attr == "set_flag":
                if call.args:
            ...
            if isinstance(
                    call.func,
                    ast.Attribute) and call.func.attr == "set_output_type":
    ...
    def visit_If(self, node: ast.If) -> ast.AST | None:
        ...
        test = node.test
        gating_attrs = {
            "platform_has_fast_fp16", "platform_has_fast_int8",
...
def _process_file(path: Path, *, write: bool) -> bool:
    ...
    if write:
        ...
        print(f"[write] {path}: create_network={m.rewrote_create_network} "
