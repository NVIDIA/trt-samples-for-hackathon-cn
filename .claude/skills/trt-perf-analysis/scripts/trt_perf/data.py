...
MODEL_COMPONENT_STOPWORDS = (MODEL_NAME_STOPWORDS -
...
def is_number(value: Any) -> bool:
    return isinstance(
        value, Number) and not isinstance(value, bool) and math.isfinite(
            float(value))
...
def suffix_for(path: str, prefix: str) -> str:
    ...
    match = re.match(rf"^{prefix}s?[_-]?(.*)\.json$",
                     base,
                     flags=re.IGNORECASE)
    if match:
...
def discover_backends(
        folder: str) -> List[Tuple[str, Optional[str], Optional[str]]]:
    layers: Dict[str, str] = {}
    ...
    for name in names:
        if not name.lower().endswith(".json"):
            continue
        layer_match = re.match(r"^layers?[_-]?(.*)\.json$",
                               name,
                               flags=re.IGNORECASE)
        profile_match = re.match(r"^profiles?[_-]?(.*)\.json$",
                                 name,
                                 flags=re.IGNORECASE)
        if layer_match:
            layers[layer_match.group(1)
                   or "default"] = os.path.join(folder, name)
        elif profile_match:
            profiles[profile_match.group(1)
                     or "default"] = os.path.join(folder, name)

    backends = [(key, layers.get(key), profiles.get(key))
                for key in sorted(set(layers) | set(profiles))]
    if not backends:
        raise DataError(
            f"No layers_*.json or profile_*.json files found in {folder}.")
    return backends


def explicit_backends(
    data_specs: Sequence[Sequence[str]]
) -> List[Tuple[str, Optional[str], Optional[str]]]:
    backends: List[Tuple[str, Optional[str], Optional[str]]] = []
    for spec in data_specs:
        if len(spec) not in (1, 2):
            raise DataError(
                "--data expects one layer-info path and optional one profile JSON path"
            )
        layer_spec = spec[0]
        ...
        profile_path = os.path.abspath(
            profile_spec) if profile_spec is not None else None
        backends.append((suffix_for(layer_path,
                                    "layer"), layer_path, profile_path))
    return backends
...
def validate_tensor(
    ...
    if not isinstance(tensor, dict):
        add_limited(
            errors,
            f"Layer `{layer_name}` {field_name}[{index}] is not an object.")
        return
    name = tensor.get("Name")
    if not isinstance(name, str):
        add_limited(
            errors,
            f"Layer `{layer_name}` {field_name}[{index}] is missing string `Name`."
        )
    dims = tensor.get("Dimensions")
    if not isinstance(dims, list) or not all(is_number(dim) for dim in dims):
        add_limited(
            errors,
            f"Layer `{layer_name}` tensor `{name}` has invalid `Dimensions`.")
    dtype = tensor.get("Format/Datatype")
    if dtype is not None and not isinstance(dtype, str):
        add_limited(
            errors,
            f"Layer `{layer_name}` tensor `{name}` has non-string `Format/Datatype`."
        )
...
def validate_layers(data: Any, errors: List[str]) -> List[Dict[str, Any]]:
    ...
    for idx, layer in enumerate(data):
        ...
        if not isinstance(name, str) or not name:
            add_limited(
                errors,
                f"Layer record {idx} is missing non-empty string `Name`.")
            name = f"<invalid-layer-{idx}>"
        names[name] += 1
        if not isinstance(layer.get("LayerType"), str):
            add_limited(errors,
                        f"Layer `{name}` is missing string `LayerType`.")
        for field_name in ("Inputs", "Outputs"):
            tensors = layer.get(field_name)
            if not isinstance(tensors, list):
                add_limited(errors,
                            f"Layer `{name}` is missing list `{field_name}`.")
                continue
    ...
    if duplicates:
        add_limited(
            errors, "Duplicate layer names: " +
            ", ".join(short_name(name, 64) for name in duplicates[:8]))
...
def has_detailed_layer_metadata(layer: Any) -> bool:
    ...
    for field_name in ("Inputs", "Outputs", "Constants"):
        ...
        if isinstance(tensors, list) and any(
                has_detailed_tensor_metadata(tensor) for tensor in tensors):
            return True
    return False
...
def has_detailed_layer_info(data: Any) -> bool:
    ...
    return isinstance(data, list) and any(
        has_detailed_layer_metadata(layer) for layer in data)
...
def backend_validation(
    ...
    return {
        ...
        "messages":
        [message("error", item)
         for item in errors] + [message("warning", item) for item in warnings],
        "source_paths": {
            "layer": layer_path,
...
def validate_profile(
        data: Any,
        errors: List[str]) -> Tuple[List[Dict[str, Any]], Optional[int], str]:
    if not isinstance(data, list):
    ...
    if isinstance(data[0], dict) and set(data[0].keys()) == {"count"}:
        ...
        if isinstance(raw_count, int) and raw_count > 0:
            ...
            add_limited(errors,
                        "Profile count header must be a positive integer.")
        entries = data[1:]
    ...
    for idx, entry in enumerate(entries):
        ...
        if not isinstance(name, str) or not name:
            add_limited(
                errors,
                f"Profile record {idx} is missing non-empty string `name`.")
        ...
        for field_name in ("timeMs", "averageMs", "medianMs", "percentage"):
            if not is_number(entry.get(field_name)):
                add_limited(
                    errors,
                    f"Profile record `{short_name(name, 72)}` has invalid `{field_name}`."
                )
        valid_entries.append(entry)

    duplicates = [
        name for name, duplicate_count in names.items() if duplicate_count > 1
    ]
    if duplicates:
        add_limited(
            errors, "Duplicate profile names: " +
            ", ".join(short_name(name, 64) for name in duplicates[:8]))

    return valid_entries, count, count_source


def validate_names(layers: Sequence[Dict[str, Any]],
                   profile: Sequence[Dict[str,
                                          Any]], errors: List[str]) -> None:
    layer_names = {
        layer.get("Name")
        for layer in layers if isinstance(layer.get("Name"), str)
    }
    profile_names = {
        entry.get("name")
        for entry in profile if isinstance(entry.get("name"), str)
    }
    missing_profile = sorted(layer_names - profile_names)
    extra_profile = sorted(profile_names - layer_names)
    if missing_profile:
        sample = ", ".join(
            short_name(name, 72) for name in missing_profile[:6])
        add_limited(
            errors,
            f"{len(missing_profile)} layer names are missing from profile JSON: {sample}"
        )
    if extra_profile:
        sample = ", ".join(short_name(name, 72) for name in extra_profile[:6])
        add_limited(
            errors,
            f"{len(extra_profile)} profile names are missing from layer-info JSON: {sample}"
        )


def tensor_name(tensor: Any) -> Optional[str]:
    if isinstance(tensor, dict) and isinstance(tensor.get("Name"),
                                               str) and tensor.get("Name"):
        return tensor["Name"]
    return None


def validate_dag(layers: Sequence[Dict[str, Any]],
                 errors: List[str]) -> Dict[str, Any]:
    producer: Dict[str, int] = {}
    duplicate_outputs: List[str] = []
    for idx, layer in enumerate(layers):
        for tensor in layer.get("Outputs", []) if isinstance(
                layer.get("Outputs"), list) else []:
    ...
    if duplicate_outputs:
        sample = ", ".join(
            short_name(name, 72) for name in duplicate_outputs[:6])
        add_limited(
            errors,
            f"Duplicate tensor producers make the graph ambiguous: {sample}")
    ...
    for consumer_idx, layer in enumerate(layers):
        for tensor in layer.get("Inputs", []) if isinstance(
                layer.get("Inputs"), list) else []:
            name = tensor_name(tensor)
            if not name:
                continue
            ...
            if producer_idx == consumer_idx:
                add_limited(
                    errors,
                    f"Layer `{short_name(layer.get('Name', '<unknown>'), 72)}` consumes its own output `{name}`."
                )
                continue
    ...
    queue = collections.deque(idx for idx, degree in enumerate(indegree)
                              if degree == 0)
    visited = 0
    while queue:
        idx = queue.popleft()
    ...
    if visited != len(layers):
        unresolved = [
            ...
            for idx, degree in enumerate(indegree) if degree > 0
        ][:8]
        add_limited(
            errors, "Layer graph is not a DAG; unresolved nodes: " +
            ", ".join(unresolved))
...
def infer_count(
        ...
        header_count: Optional[int]) -> Tuple[Optional[int], str, List[str]]:
    warnings: List[str] = []
    ...
    for entry in profile:
        ...
        if is_number(time_ms) and is_number(average_ms) and float(
                average_ms) > 0:
    ...
    if ratios:
        ...
        if rounded > 0:
            ...
            if max_delta <= max(0.05 * rounded, 0.5):
                ...
                warnings.append(
                    "Profile timeMs/averageMs ratios are not consistent enough to infer iteration count."
                )

    if header_count is not None:
        if inferred is not None and abs(inferred - header_count) > max(
                1, int(0.05 * header_count)):
            warnings.append(
                f"Profile count header ({header_count}) disagrees with inferred ratio ({inferred})."
            )
...
def validate_timing(profile: Sequence[Dict[str, Any]], count: Optional[int],
                    warnings: List[str]) -> Dict[str, float]:
    total_time = sum(
        float(entry["timeMs"]) for entry in profile
        if is_number(entry.get("timeMs")))
    total_average = sum(
        float(entry["averageMs"]) for entry in profile
        if is_number(entry.get("averageMs")))
    total_percentage = sum(
        float(entry["percentage"]) for entry in profile
        if is_number(entry.get("percentage")))

    if abs(total_percentage - 100.0) > 2.0:
        warnings.append(
            f"Profile percentages sum to {total_percentage:.2f}%, not approximately 100%."
        )

    if count and total_average > 0:
        expected_time = total_average * count
        if abs(expected_time - total_time) > max(
                0.01, 0.02 * max(total_time, expected_time)):
            warnings.append(
    ...
    return {
        ...
        total_average if total_average else
        (total_time / count if count else 0.0),
        "total_percentage":
        total_percentage,
    }
...
def model_name_tokens(value: str) -> List[str]:
    return [
        token for token in normalized_name_tokens(value)
        if token not in MODEL_NAME_STOPWORDS
    ]
...
def clean_model_name(value: Any) -> Optional[str]:
    ...
    segments = [
        segment.strip() for segment in re.split(r"[\\/]+", ascii_value)
        if segment.strip()
    ]
    if not segments:
...
def infer_model_name_from_configs(
        folder: Optional[str]) -> List[ModelNameGuess]:
        ...
        for key in CONFIG_MODEL_NAME_KEYS:
            ...
            if cleaned:
                guesses.append(
                    ModelNameGuess(
                        cleaned, "high",
                        f"config `{os.path.basename(path)}` field `{key}`"))
                break
    return guesses


def infer_model_name_from_folder(
        folder: Optional[str]) -> List[ModelNameGuess]:
    if not folder:
        return []
    ...
    if parts and parts[-1].lower() in MODEL_NAME_PARENT_HINT_DIRS and len(
            parts) >= 2:
    ...
    for part in candidate_parts:
        ...
        if cleaned:
            guesses.append(
                ModelNameGuess(cleaned, "medium", f"directory name `{part}`"))
            break
    return guesses


def infer_model_name_from_filenames(
        source_paths: Optional[Sequence[str]]) -> List[ModelNameGuess]:
    guesses: List[ModelNameGuess] = []
    for path in source_paths or []:
        base = os.path.splitext(os.path.basename(path))[0]
        stripped = re.sub(r"^(layers?|profiles?)[_-]*",
                          "",
                          base,
                          flags=re.IGNORECASE)
        ...
        if cleaned:
            guesses.append(
                ModelNameGuess(cleaned, "medium",
                               f"input file `{os.path.basename(path)}`"))
    return guesses
...
def infer_model_name_metadata(
    ...
    if explicit_cleaned:
        guesses.append(
            ModelNameGuess(explicit_cleaned, "high", "explicit model name"))
    ...
    best = max(guesses,
               key=lambda item: MODEL_NAME_CONFIDENCE_RANK[item.confidence])
...
def clean_model_component_segment(value: str,
    ...
    if model_tokens:
        without_model = [
            token for token in tokens if token not in model_tokens
        ]
        if any(token in MODEL_COMPONENT_HINT_TOKENS
               for token in without_model):
...
def append_unique_component(components: List[str],
...
def infer_model_components(
    ...
    if folder:
        ...
        append_unique_component(
            components, clean_model_component_segment(folder_name, model_name))
    ...
    for path in source_paths or []:
        ...
        stripped = re.sub(r"^(layers?|profiles?)[_-]*",
                          ...
                          flags=re.IGNORECASE)
        append_unique_component(
            components, clean_model_component_segment(stripped, model_name))

    return components


def collect_shapes(
        layers: Sequence[Dict[str, Any]]) -> List[Tuple[float, ...]]:
    shapes: List[Tuple[float, ...]] = []
    for layer in layers:
        for field_name in ("Inputs", "Outputs"):
            for tensor in layer.get(field_name, []) if isinstance(
                    layer.get(field_name), list) else []:
                dims = tensor.get("Dimensions") if isinstance(tensor,
                                                              dict) else None
                if isinstance(dims, list) and all(
                        is_number(dim) for dim in dims):
                    shapes.append(tuple(float(dim) for dim in dims))
    return shapes


def external_input_shapes(
        layers: Sequence[Dict[str, Any]],
        external_inputs: Iterable[str]) -> Dict[str, Tuple[float, ...]]:
    ...
    for layer in layers:
        for tensor in layer.get("Inputs", []) if isinstance(
            ...
            dims = tensor.get("Dimensions") if isinstance(tensor,
                                                          dict) else None
            if name in external and isinstance(dims, list) and all(
                    is_number(dim) for dim in dims):
...
def infer_model_info(
    ...
    if config:
        ...
        if model_type in {"bert", "roberta", "distilbert", "albert", "mpnet"
                          } or "bert" in architectures:
            category = "Transformer encoder / sentence embedding"
            confidence = "high"
            evidence.append(
                f"Hugging Face config model_type={config.get('model_type')}")
        elif any(token in model_type
                 for token in ("gpt", "llama", "mistral", "gemma", "decoder")):
    ...
    if confidence == "low":
        if "input_ids" in names_blob and ("encoder.layer" in names_blob
                                          or "attention" in names_blob):
            category = "Transformer encoder / sentence embedding"
            confidence = "medium"
            evidence.append(
                "Layer names include input_ids and attention/encoder markers")
    ...
    last_dims = collections.Counter(
        int(shape[-1]) for shape in shapes
    ...
    if intermediate_size is None and hidden_size and last_dims:
        larger = [
            dim for dim, count in last_dims.items()
            if dim > int(hidden_size) and count >= 2
        ]
        if larger:
    ...
    if num_layers is None:
        encoder_indices = [
            int(match)
            for match in re.findall(r"encoder\.layer\.(\d+)", names_blob)
        ]
        if encoder_indices:
            num_layers = max(encoder_indices) + 1

    input_shapes = external_input_shapes(layers,
                                         graph.get("external_inputs", []))
    sequence_lengths = []
    for name, shape in input_shapes.items():
        if any(token in name
               for token in ("input_ids", "attention_mask",
                             "token_type_ids")) and len(shape) >= 2:
            sequence_lengths.append(int(shape[-1]))
    sequence_length = max(
        set(sequence_lengths),
        key=sequence_lengths.count) if sequence_lengths else None
...
def engine_hints(layers: Sequence[Dict[str, Any]],
...
def combined_layer_text(layer: Dict[str, Any]) -> str:
    return " ".join(
        str(layer.get(field, ""))
        for field in ("Name", "LayerType", "TacticName", "Metadata")).lower()
...
def tags_for(layer: Dict[str, Any]) -> List[str]:
    ...
    } or "matmul" in combined or "matrix_multiply" in combined:
        tags.append("matmul/gemm")
    if "attention" in combined or "fmha" in combined or "flash" in combined or looks_like_mha(
            layer):
        tags.append("attention")
    if "scaled_dot_product_attention" in combined:
        tags.append("sdpa")
    if any(token in lower
           for token in DYNAMIC_TOKENS) or layer_type == "shape_call":
...
def is_qkv_fused(layer: Dict[str, Any]) -> bool:
    ...
    has_named_qkv = all(token in combined
                        for token in ("query", "key", "value"))
    has_three_matmuls = combined.count("matmul") >= 3 or combined.count(
        "matrix_multiply") >= 3
...
def is_obvious_attention_fusion(layer: Dict[str, Any]) -> bool:
    combined = (str(layer.get("Name", "")) + " " +
                str(layer.get("Metadata", ""))).lower()
    return any(token in combined
               for token in ("scaled_dot_product_attention", "fmha",
                             "flash_attention", "flashattention"))


def combine_records(layers: Sequence[Dict[str, Any]],
                    profile: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def aggregate(records: Sequence[Dict[str, Any]],
    ...
    for record in records:
        keys = record[key_name] if isinstance(record[key_name],
                                              list) else [record[key_name]]
        for key in keys:
...
def find_issues(
    ...
    if top_layers:
        ...
        if top["percentage"] >= 10.0:
            ...
            positives.append(
                f"No single layer exceeds 5%; the largest layer is {fmt_pct(top['percentage'])}."
            )
    ...
    qkv_fused_count = sum(1 for record in records
                          if is_qkv_fused(record["layer"]))
    if is_transformer and isinstance(block_count, int) and block_count > 0:
        ...
        attention_fused_count = sum(
            1 for record in records
            if is_obvious_attention_fusion(record["layer"]))
        if attention_fused_count < block_count:
            ...
            positives.append(
                f"Detected {attention_fused_count} obvious fused attention layers for {block_count} transformer blocks."
            )
    ...
    if isinstance(engine_count, int):
        if engine_count > 1:
            ...
            positives.append(
                f"Engine hints point to one engine ({engines.get('source')}).")
...
def failure_result(
    ...
    result: Dict[str, Any] = {
        ...
        label,
        "validation":
        backend_validation("failed", "unusable", errors, warnings, layer_path,
                           profile_path, False),
    }
...
def analyze_backend(
    ...
    if not layer_path:
        ...
        return failure_result(label, errors, warnings, layer_path,
                              profile_path)

    try:
        layer_data = load_json(layer_path)
    except DataError as exc:
        return failure_result(label, [str(exc)], warnings, layer_path,
                              profile_path)
    ...
    graph = validate_dag(layers, errors) if layers else {
        "edge_count": 0,
        "external_inputs": [],
        "graph_outputs": []
    }
    ...
    if not profile_path:
        if errors:
            return failure_result(label, errors, warnings, layer_path,
                                  profile_path, len(layers))
        model = infer_model_info(layers, graph, folder, model_name_metadata,
                                 model_components)
        ...
        return {
            ...
            label,
            "validation":
            backend_validation("passed", "layer_only", errors, warnings,
                               layer_path, profile_path, True),
            "layers":
            layers,
            "layer_count":
            len(layers),
    ...
    profile, header_count, count_source = validate_profile(
        profile_data, errors)
    if not errors:
    ...
    if errors:
        return failure_result(label, errors, warnings, layer_path,
                              profile_path, len(layers), len(profile))
    ...
    issues, positives = find_issues(records, model, engines, type_breakdown,
                                    tag_breakdown)
    ...
    return {
        ...
        label,
        "validation":
        backend_validation("passed", "layer_profile", errors, warnings,
                           layer_path, profile_path, True),
        "layers":
        layers,
        "profile":
...
def extract_perf_data(
    ...
    if data_specs:
        if path:
            raise DataError(
                "Use either a folder path or explicit --data inputs, not both."
            )
        backends = explicit_backends(data_specs)
        source_paths = [
            source_path for _, layer_path, profile_path in backends
            for source_path in (layer_path, profile_path) if source_path
        ]
    ...
    model_name_metadata = infer_model_name_metadata(folder, model_name,
                                                    source_paths)
    model_components = infer_model_components(folder, source_paths,
                                              model_name_metadata["name"])
    results = [
        analyze_backend(label, layer_path, profile_path, folder,
                        model_name_metadata, model_components)
