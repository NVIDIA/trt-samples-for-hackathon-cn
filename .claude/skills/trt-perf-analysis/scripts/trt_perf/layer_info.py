...
def load_layers(path: Path) -> list[dict[str, Any]]:
    ...
    bad_indexes = [
        idx for idx, layer in enumerate(layers) if not isinstance(layer, dict)
    ]
    if bad_indexes:
        preview = ", ".join(str(idx) for idx in bad_indexes[:5])
        raise ValueError(
            f"layer entries must be objects; invalid indexes: {preview}")
...
def kgen_details(layer: dict[str, Any]) -> tuple[str, list[str], str]:
    ...
    for candidate in candidates:
        ...
        tokens = [
            normalize_kgen_token(token)
            for token in split_kgen_signature(signature)
        ]
        if tokens:
...
def process_layers(layers: list[dict[str, Any]],
                   source_path: Path) -> ProcessedLayerInfo:
    ...
    for index, layer in enumerate(layers):
        ...
        extra = {
            key: value
            for key, value in layer.items() if key not in STANDARD_LAYER_KEYS
        }
    ...
    terminal_values = [
        value_name for value_name in produced_values
        if value_name not in consumed_values
    ]
    ...
    graph_value_info = [
        value_name for value_name in descriptors_by_value
        if value_name not in graph_input_names
        and value_name not in graph_output_names
    ]
...
def resolve_target_spec(target: str) -> TargetSpec:
    try:
        ...
        raise ValueError(
            f"unknown target {target!r}; valid targets: {valid}") from exc
...
def write_target(processed: ProcessedLayerInfo, spec: TargetSpec,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=
        "Process TensorRT layer-info JSON and write one or more visualization targets."
    )
    parser.add_argument("input",
                        type=Path,
                        nargs="?",
                        help="Path to TensorRT layer-info JSON")
    parser.add_argument(
        "-t",
        "--target",
        action="append",
        ...
        metavar="TARGET",
        help=
        f"Output target; repeat for multiple outputs. Default: html. Available: {target_names_help()}",
    )
    parser.add_argument(
        "-o",
        "--output",
...
def list_targets() -> None:
    ...
    alias_only = sorted(name for name, spec in TARGETS_BY_NAME.items()
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.input is None:
        print("error: input is required unless --list-targets is used",
              file=sys.stderr)
        return 1
    ...
    try:
        ...
        if args.output is not None and len(target_specs) != 1:
            raise ValueError(
                "--output can only be used when writing one target")
