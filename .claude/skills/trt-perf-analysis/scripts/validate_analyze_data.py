...
def default_schema_path() -> str:
    ...
    return os.path.abspath(
        os.path.join(script_dir, "..", "schemas", "analyze-data.schema.json"))
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=
        "Validate TensorRT analysis JSON generated by analyze_trt_perf.py.")
...
def resolve_ref(ref: str, root_schema: Any) -> Any:
    if not ref.startswith("#/"):
        raise ValidationError(
            f"only local JSON Schema refs are supported: {ref}")
...
def append_nearest_errors(errors: List[str],
                          matches: List[List[str]],
                          limit: int = 5) -> None:
    if not matches:
        return
    nearest = min(matches,
                  key=lambda item: (sum("expected constant" in error
                                        for error in item), len(item)))
    for error in nearest[:limit]:
        errors.append(f"  nearest schema: {error}")


def validate(instance: Any, schema: Any, path: str,
             root_schema: Any) -> List[str]:
    if not isinstance(schema, dict):
        raise ValidationError(f"{path}: schema node is not an object")

    if "$ref" in schema:
        return validate(instance, resolve_ref(str(schema["$ref"]),
                                              root_schema), path, root_schema)
    ...
    if "anyOf" in schema:
        matches = [
            validate(instance, subschema, path, root_schema)
    ...
    if "oneOf" in schema:
        ...
        if match_count != 1:
            append_error(
                errors, path,
                f"matches {match_count} schemas, expected exactly one")
            if match_count == 0:
                append_nearest_errors(errors, matches)
    ...
    if "const" in schema and instance != schema["const"]:
        append_error(
            errors, path,
            f"expected constant {schema['const']!r}, got {instance!r}")
    ...
    if expected_type is not None and not matches_any_type(
            instance, expected_type):
        append_error(
            errors, path,
            f"expected type {expected_type!r}, got {type_label(instance)}")
        return errors
...
def validate_object(instance: dict, schema: dict, path: str, root_schema: Any,
    ...
    for key, value in instance.items():
        ...
        if key in properties:
            errors.extend(
                validate(value, properties[key], child_path, root_schema))
        elif additional is False:
            append_error(errors, child_path,
                         "additional property is not allowed")
        elif isinstance(additional, dict):
            errors.extend(validate(value, additional, child_path, root_schema))


def validate_array(instance: list, schema: dict, path: str, root_schema: Any,
                   errors: List[str]) -> None:
    min_items = schema.get("minItems")
    if isinstance(min_items, int) and len(instance) < min_items:
        append_error(
            errors, path,
            f"expected at least {min_items} items, got {len(instance)}")

    max_items = schema.get("maxItems")
    if isinstance(max_items, int) and len(instance) > max_items:
        append_error(
            errors, path,
            f"expected at most {max_items} items, got {len(instance)}")

    items_schema = schema.get("items")
    if isinstance(items_schema, dict):
        for index, value in enumerate(instance):
            errors.extend(
                validate(value, items_schema, f"{path}[{index}]", root_schema))


def validate_number(instance: float, schema: dict, path: str,
                    errors: List[str]) -> None:
    if "minimum" in schema and instance < schema["minimum"]:
        append_error(errors, path,
                     f"expected >= {schema['minimum']}, got {instance}")
    if "maximum" in schema and instance > schema["maximum"]:
        append_error(errors, path,
                     f"expected <= {schema['maximum']}, got {instance}")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if errors:
        sys.stderr.write(
            f"{args.input}: FAIL ({len(errors)} schema error(s))\n")
