...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=
        "Generate and package a standalone TensorRT performance report.")
    parser.add_argument(
        "path",
        nargs="?",
        help=
        "Folder directly containing layers_*.json and/or profile_*.json inputs. Pass one component/report folder per invocation. Omit when using --analyze-data.",
    )
    parser.add_argument(
        "--analyze-data",
        help=
        "Existing analyze-data.json to validate and package in place. The file must be named analyze-data.json.",
    )
    parser.add_argument(
        "--output-parent",
        help=
        "Directory where an auto-named report folder should be created. Defaults to the current working directory unless that is inside the skill.",
    )
    parser.add_argument(
        "--report-dir",
        help=
        "Exact report directory to create. For new analysis this directory must be empty or absent; useful for component-specific reports.",
    )
    parser.add_argument(
        "--model-name",
        help=
        "Model name from the user prompt or caller context. It is cleaned and stored in analyze-data.json.",
    )
    parser.add_argument(
        "--timestamp",
        help=
        "Timestamp for auto-generated report folder names, formatted as YYYYMMDD_HHMMSS. Defaults to local time.",
    )
    parser.add_argument(
        "--template-dir",
        default=str(default_template_dir()),
        help=
        "Report template directory. Defaults to report_template in this skill.",
    )
    parser.add_argument(
        "--analyze-md",
        help=
        "AI-generated Markdown analysis to copy into the report folder as analyze.md after the final analysis is written.",
    )
...
def inferred_analyzer_model_name(
        input_path: Path, explicit_name: Optional[str]) -> Optional[str]:
    metadata = infer_model_name_metadata(str(input_path), explicit_name,
                                         input_source_paths(input_path))
    return metadata["name"]


def infer_report_model_name(input_path: Path,
                            explicit_name: Optional[str]) -> str:
    metadata = infer_model_name_metadata(str(input_path), explicit_name,
                                         input_source_paths(input_path))
...
def next_available_report_dir(output_parent: Path, model_name: str,
                              timestamp: str) -> Path:
...
def resolve_new_report_dir(args: argparse.Namespace, input_path: Path) -> Path:
    ...
    output_parent = Path(args.output_parent).expanduser(
    ) if args.output_parent else default_output_parent()
...
def validate_template_dir(template_dir: Path) -> None:
    ...
    missing = [
        name for name in required if not (template_dir / name).is_file()
    ]
    if missing:
        raise ValueError(
            f"report template is missing required file(s): {', '.join(missing)}"
        )
...
def copy_analyze_markdown(analyze_md: Optional[str],
...
def package_existing_json(args: argparse.Namespace) -> int:
    ...
    if not analyze_data_path.is_file():
        sys.stderr.write(
            f"error: analyze-data file not found: {analyze_data_path}\n")
        return 2
    if analyze_data_path.name != "analyze-data.json":
        sys.stderr.write(
            "error: --analyze-data must point to a file named analyze-data.json\n"
        )
        return 2
    ...
    try:
        copy_template(
            Path(args.template_dir).expanduser(), analyze_data_path.parent)
        analyze_markdown_path = copy_analyze_markdown(args.analyze_md,
                                                      analyze_data_path.parent)
...
def package_new_analysis(args: argparse.Namespace) -> int:
    if not args.path:
        sys.stderr.write(
            "error: provide an input folder, or use --analyze-data\n")
        return 2
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.path and args.analyze_data:
        sys.stderr.write(
            "error: use either an input folder or --analyze-data, not both\n")
        return 2
