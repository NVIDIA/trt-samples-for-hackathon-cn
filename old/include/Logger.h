...
public:
  ...
  virtual char *GetLead(LogLevel l, const char *szFile, int nLine,
                        const char *szFunc) {
    if (l < TRACE || l > FATAL) {
      sprintf(szLead, "[?????] ");
      return szLead;
...
private:
  ...
  public:
    ...
    std::ostream &GetStream() override { return *pFileOut; }
    void FlushStream() override { pFileOut->flush(); }
  ...
  public:
    ...
    UdpOstream &GetStream() override { return udpOut; }
    void FlushStream() override { udpOut.Flush(); }
...
#define LOG(level)                                                             \
  simplelogger::LogTransaction(logger, simplelogger::level, __FILE__,          \
