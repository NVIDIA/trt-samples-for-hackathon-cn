...
public:
  void log(Severity severity, const char *msg) noexcept override {
    static simplelogger::LogLevel map[] = {
        simplelogger::FATAL, simplelogger::ERROR, simplelogger::WARNING,
        simplelogger::INFO, simplelogger::TRACE};
    simplelogger::LogTransaction(logger, map[(int)severity], __FILE__, __LINE__,
                                 __FUNCTION__)
            .GetStream()
        << msg;
  }
...
typedef void (*ConfigBuilderProcType)(IBuilderConfig *config,
...
struct IOInfo {
  ...
  string GetDimString() { return ::to_string(dim); }
  ...
  string to_string() {
    ostringstream oss;
    oss << setw(6) << (bInput ? "input" : "output") << " | " << setw(5)
        << GetDataTypeString() << " | " << GetDimString() << " | "
...
public:
  ...
  TrtLite *Clone() {
    if (!engine->hasImplicitBatchDimension() &&
        *pnProfile == engine->getNbOptimizationProfiles()) {
  ...
  ICudaEngine *GetEngine() { return engine.get(); }
  void Execute(int nBatch, vector<void *> &vdpBuf, cudaStream_t stm = 0,
               cudaEvent_t *evtInputConsumed = nullptr) {
    if (!engine->hasImplicitBatchDimension() && nBatch > 1) {
      LOG(WARNING) << "Engine was built with explicit batch but is executed "
                      "with batch size != 1. Results may be incorrect.";
      return;
    }
  ...
  void Execute(map<int, Dims> i2shape, vector<void *> &vdpBuf,
               cudaStream_t stm = 0, cudaEvent_t *evtInputConsumed = nullptr) {
    if (engine->hasImplicitBatchDimension()) {
    ...
    // i2shape may have different size with nBuf, because some shapes don't need
    // it or have been set
    for (auto &it : i2shape) {
      context->setBindingDimensions(it.first + nBuf * iProfile, it.second);
    }
    vector<void *> vdpBufEnqueue(engine->getNbBindings());
    std::copy(vdpBuf.begin(), vdpBuf.end(),
              vdpBufEnqueue.begin() + nBuf * iProfile);
  ...
  void Refit(vector<tuple<const char *, WeightsRole, Weights>> vtWeight) {
    ...
    for (auto tWeight : vtWeight) {
      if (!refitter->setWeights(get<0>(tWeight), get<1>(tWeight),
                                get<2>(tWeight))) {
  ...
  vector<IOInfo> ConfigIO(int nBatchSize) {
    ...
    if (!engine->hasImplicitBatchDimension()) {
      LOG(ERROR)
          << "Engine must be built with implicit batch size (and static shape)";
      return vInfo;
    }
    for (int i = 0; i < engine->getNbBindings(); i++) {
      vInfo.push_back({string(engine->getBindingName(i)),
                       engine->bindingIsInput(i),
                       MakeDim(nBatchSize, engine->getBindingDimensions(i)),
                       engine->getBindingDataType(i)});
    }
    return vInfo;
  }
  vector<IOInfo> ConfigIO(map<int, Dims> i2shape) {
    ...
    if (engine->hasImplicitBatchDimension()) {
      LOG(ERROR) << "Engine must be built with explicit batch size (to enable "
                    "dynamic shape)";
      return vInfo;
    }
    if (!context) {
    ...
    for (int i = nBuf * iProfile; i < nBuf * iProfile + nBuf; i++) {
      vInfo.push_back(
          {string(engine->getBindingName(i)), engine->bindingIsInput(i),
  ...
  void PrintInfo() {
    ...
    // Only IO information at the engine level is included: -1 for dynamic shape
    // dims
    for (size_t i = 0; i < engine->getNbBindings(); i++) {
      cout << "#" << i << ": "
           << IOInfo{string(engine->getBindingName(i)),
                     engine->bindingIsInput(i), engine->getBindingDimensions(i),
                     engine->getBindingDataType(i)}
                  .to_string()
           << endl;
    }
...
private:
  TrtLite(TrtLogger trtLogger, ICudaEngine *engine)
      : trtLogger(trtLogger), engine(shared_ptr<ICudaEngine>(engine)),
        pnProfile(make_shared<int>()) {
    *pnProfile = 1;
  }
  TrtLite(TrtLite const &trt) = default;

  static size_t GetBytesOfBinding(int iBinding, ICudaEngine *engine,
                                  IExecutionContext *context = nullptr) {
    size_t aValueSize[] = {4, 2, 1, 4, 1};
    size_t nSize = aValueSize[(int)engine->getBindingDataType(iBinding)];
    const Dims &dims = context ? context->getBindingDimensions(iBinding)
                               : engine->getBindingDimensions(iBinding);
  ...
  IExecutionContext *context = nullptr;
  int iProfile = 0;
  /*It seems Builder's logger will be passed to Engine, but there's no API to
    extract it out. Refitter is created from Engine, however, its logger should
    be passed explicitly but must be the same as Engine's. So it's a good idea
    to keep one logger for all and its life cycle should be at least as longer
    as Engine's. Besides, multi-threading on TRT 7.2 may yields mixed log
    message.*/
  TrtLogger trtLogger;
...
public:
  ...
  static TrtLite *
  CreateFromOnnx(const char *szOnnxPath,
                 ConfigBuilderProcType ConfigBuilderProc = nullptr,
                 int nProfile = 0, void *pData = nullptr) {
    ...
    for (int i = 0; i < network->getNbInputs(); i++) {
      ...
      cout << "#" << i << ": "
           << IOInfo{string(tensor->getName()), true, tensor->getDimensions(),
                     tensor->getType()}
                  .to_string()
           << endl;
    }
