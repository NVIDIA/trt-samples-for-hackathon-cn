...
int AddPlugin::enqueue(int nBatch, void const *const *inputs,
  ...
  if (m.dataType == nvinfer1::DataType::kFLOAT) {
    ...
    std::cout << "valueToAdd (int8 scaled): " << valueToAdd << ", "
              << (int)valueToAdd << std::endl;
    AddValue<<<(n + 1023) / 1024, 1024>>>(
        (int8_t *)outputs[0], (int8_t *)inputs[0], n, (int8_t)valueToAdd);
  }
  return 0;
}
