...
__global__ void OnehotPluginKernel(int *pArgmax, float *pOnehotPluginEncode,
                                   int depth) {
...
int OnehotPlugin::enqueue(int batchsize, const void *const *inputs,
  ...
  // std::cout << "dimBlock" << dimBlock.x << ", dimGrid: " << dimGrid.x << ", "
  // << dimGrid.y << std::endl;

  OnehotPluginKernel<<<dimGrid, dimBlock, 0, stream>>>(
      (int *)inputs[0], (float *)outputs[0], m.depth);
  return 0;
}
