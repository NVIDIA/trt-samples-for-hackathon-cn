...
inline bool check(cudnnStatus_t e, int iLine, const char *szFile) {
  if (e != CUDNN_STATUS_SUCCESS) {
    std::cerr << "CUDA driver API error " << cudnnGetErrorString(e)
...
public:
  ...
  nvinfer1::DimsExprs
  getOutputDimensions(int index, const nvinfer1::DimsExprs *pInputDim,
                      int nInputDim,
...
private:
  ...
  template <class T>
  void SpatialTfSamplerForward(const nvinfer1::PluginTensorDesc *inputDesc,
                               const nvinfer1::PluginTensorDesc *outputDesc,
...
nvinfer1::PluginFieldCollection GridSamplerPluginCreator::fc =
    nvinfer1::PluginFieldCollection();

template <class T>
void GridSamplerPlugin::SpatialTfSamplerForward(
    const nvinfer1::PluginTensorDesc *inputDesc,
    const nvinfer1::PluginTensorDesc *outputDesc, const void *const *inputs,
    void *const *outputs, cudnnDataType_t cudnnDataType) {
  ck(cudnnSetTensorNdDescriptorEx(xDesc, CUDNN_TENSOR_NCHW, cudnnDataType,
                                  inputDesc[0].dims.nbDims,
                                  inputDesc[0].dims.d));
  ck(cudnnSetTensorNdDescriptorEx(yDesc, CUDNN_TENSOR_NCHW, cudnnDataType,
                                  outputDesc[0].dims.nbDims,
                                  outputDesc[0].dims.d));
  ck(cudnnSetSpatialTransformerNdDescriptor(
      stDesc, CUDNN_SAMPLER_BILINEAR, cudnnDataType, inputDesc[0].dims.nbDims,
      inputDesc[0].dims.d));

  T alpha = 1.0f, beta = 0.0f;
  ck(cudnnSpatialTfSamplerForward(cudnnHandle, stDesc, &alpha, xDesc, inputs[0],
                                  inputs[1], &beta, yDesc, outputs[0]));
}
...
int GridSamplerPlugin::enqueue(const nvinfer1::PluginTensorDesc *inputDesc,
  ...
  if (inputDesc[0].type == nvinfer1::DataType::kFLOAT) {
    SpatialTfSamplerForward<float>(inputDesc, outputDesc, inputs, outputs,
                                   CUDNN_DATA_FLOAT);
  } else if (inputDesc[1].type == nvinfer1::DataType::kHALF) {
    SpatialTfSamplerForward<half>(inputDesc, outputDesc, inputs, outputs,
                                  CUDNN_DATA_HALF);
  } else {
    std::cerr << "Unsupported data type: " << (int)inputDesc[1].type
              << std::endl;
  }
...
// Assuming TRT is installed to /usr/local/tensorrt7 and you are in the upper
// directory, complile with one of the following:
// nvcc -g -Xcompiler -fPIC -shared -o GridSamplerPlugin.so
// plugins/GridSamplerPlugin.cpp -isystem /usr/local/tensorrt7/include
// -L/usr/local/tensorrt7/lib -lnvinfer c++ -g -fPIC -shared -o
// GridSamplerPlugin.so plugins/GridSamplerPlugin.cpp -I/usr/local/cuda/include
// -isystem /usr/local/tensorrt7/include -L/usr/local/tensorrt7/lib -lnvinfer
