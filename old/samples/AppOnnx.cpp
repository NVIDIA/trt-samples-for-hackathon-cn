...
static void ConfigBuilderProc(IBuilderConfig *config,
  ...
  vProfile[0]->setDimensions(
      "input", OptProfileSelector::kMIN,
      Dims4(1, pParam->nChannel, pParam->nHeight, pParam->nWidth));
  vProfile[0]->setDimensions("input", OptProfileSelector::kOPT,
                             Dims4(pParam->nMaxBatchSize, pParam->nChannel,
                                   pParam->nHeight, pParam->nWidth));
  vProfile[0]->setDimensions("input", OptProfileSelector::kMAX,
                             Dims4(pParam->nMaxBatchSize, pParam->nChannel,
                                   pParam->nHeight, pParam->nWidth));
...
int main(int argc, char **argv) {
  ...
  i2shape.insert(make_pair(0, Dims4(param.nMaxBatchSize, param.nChannel,
                                    param.nHeight, param.nWidth)));

  int iCase = argc >= 3 ? atoi(argv[2]) : 0;
  auto trt = unique_ptr<TrtLite>(
      iCase ? TrtLiteCreator::CreateFromOnnx(
                  "../python/resnet50.dynamic_shape.onnx", ConfigBuilderProc, 1,
                  &param)
  ...
  // print((float *)vpBuf[1], param.nMaxBatchSize * param.nChannel *
  // param.nHeight, param.nWidth);
