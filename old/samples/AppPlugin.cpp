...
IHostMemory *BuildNetworkProc(IBuilder *builder, void *pData) {
  ...
  unique_ptr<INetworkDefinition> network(
      builder->createNetworkV2(bExplicitBatch ? 1 : 0));
  ITensor *tensor;
  if (bExplicitBatch) {
    tensor = network->addInput("input0", DataType::kFLOAT,
                               Dims4{pParam->nBatchSize, pParam->nChannel,
                                     pParam->nHeight, pParam->nWidth});
  } else {
    tensor = network->addInput(
