...
IHostMemory *BuildNetworkProc(IBuilder *builder, void *pData) {
  ...
  ITensor *tensor = network->addInput(szInputName, DataType::kFLOAT,
  ...
  if (pParam->bInt8) {
    // to output int8 from the plugin, it must be set explicitly, otherwise it
    // won't be selected
    tensor->setType(DataType::kINT8);
    // convert to float (if not, output type is int8 and you need to deal with
  ...
  BuildEngineParam optParam = {pParam->nMaxBatchSize, pParam->nChannel, 1024,
                               1024};
  ...
  profile->setDimensions(szInputName, OptProfileSelector::kMIN,
                         Dims4(1, pParam->nChannel, 1, 1));
  profile->setDimensions(szInputName, OptProfileSelector::kOPT,
                         Dims4(optParam.nMaxBatchSize, optParam.nChannel,
                               optParam.nHeight, optParam.nWidth));
...
int main(int argc, char **argv) {
  ...
  print((float *)vpBuf[0],
        vInfo[1].dim.d[0] * vInfo[1].dim.d[1] * vInfo[1].dim.d[2], 8);
  print((float *)vpBuf[1],
        vInfo[1].dim.d[0] * vInfo[1].dim.d[1] * vInfo[1].dim.d[2], 8);
