...
IHostMemory *BuildNetworkProc(IBuilder *builder, void *pData) {
    ...
    ITensor *shapeTensor = network->addShape(*input)->getOutput(0);
    IFillLayer *fillLayer = network->addFill(Dims(), FillOperation::kLINSPACE);
    fillLayer->setInput(0, *shapeTensor);
    float start = 0.0f;
    fillLayer->setInput(1, *network->addConstant(Dims{}, Weights{DataType::kFLOAT, &start, 1})->getOutput(0));
    float delta[] = {0, 0, 0, 1.0f};
    fillLayer->setInput(2, *network->addConstant(Dims{1, 4}, Weights{DataType::kFLOAT, delta, sizeof(delta) / sizeof(delta[0])})->getOutput(0));
    ITensor *tensor = fillLayer->getOutput(0);
