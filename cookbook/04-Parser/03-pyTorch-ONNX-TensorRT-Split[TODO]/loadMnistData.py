...
class MnistData():
    def __init__(self, dataPath, isOneHot = False):        
        with gfile.Open(dataPath+'train-images-idx3-ubyte.gz', 'rb') as f:
            self.trainImage = self.extractImage(f)
        with gfile.Open(dataPath+'train-labels-idx1-ubyte.gz', 'rb') as f:
            self.trainLabel = self.extractLabel(f)
        with gfile.Open(dataPath+'t10k-images-idx3-ubyte.gz', 'rb') as f:
            self.testImage  = self.extractImage(f)
        with gfile.Open(dataPath+'t10k-labels-idx1-ubyte.gz', 'rb') as f:
    ...
    def getBatch(self, batchSize, isTrain):
        if isTrain:
            index = index = np.random.choice(len(self.trainImage), batchSize, True)   
            return self.trainImage[index], self.trainLabel[index]
        else:
            index = index = np.random.choice(len(self.testImage), batchSize, True)    
    ...
    def saveImage(self, count, outputPath, isTrain):
        ...
        for i in range(min(count,10000)):
            cv2.imwrite(outputPath+"/"+str(i).zfill(5)+"-"+str(label[i])+".jpg", image[i])
