...
if True:
    _C1 = network.add_constant([1], np.array([5], dtype=np.int32))  # check whether inputT0.shape[3] == 5
else:
    _C1 = network.add_constant([1], np.array([4], dtype=np.int32))  # check whether inputT0.shape[3] == 4, we know it will fail at build time
...
_HA = network.add_assertion(_H4.get_output(0), "inputT0.shape[3] != 5")  # assertion layer has a Bool input tensor but has no output tensor
