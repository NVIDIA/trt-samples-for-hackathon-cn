...
_C1 = network.add_constant([1], np.array([5], dtype=np.int32))  # 检查 inputT0.shape[3] 是否等于 5
#_C1 = network.add_constant([1], np.array([4], dtype=np.int32))  # 检查 inputT0.shape[3] 是否为 4，该检查在构建期就肯定不能通过
_H3 = network.add_elementwise(_H2.get_output(0), _C1.get_output(0), trt.ElementWiseOperation.EQUAL)  # 通过 EQUAL 方法来检查
