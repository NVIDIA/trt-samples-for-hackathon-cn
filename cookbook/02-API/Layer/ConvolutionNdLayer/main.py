...
data = np.arange(nHKernel * nWKernel, dtype=np.float32).reshape(1, 1, nHKernel, nWKernel)
data = np.tile(data, (nB, nC, nH // nHKernel, nW // nWKernel)) + 1
data = {"inputT0": data}
...
def case_simple():
    ...
    layer = tw.network.add_convolution_nd(tensor, nCOut, [nHKernel, nWKernel], trt.Weights(w), trt.Weights(b))
...
def case_parameter():
    ...
    tensor = tw.network.add_input("inputT0", trt.float32, data["inputT0"].shape)
    layer = tw.network.add_deconvolution_nd(tensor, 1, [1, 1], np.zeros(1, dtype=np.float32))
...
def case_group():
    ...
    layer = tw.network.add_convolution_nd(tensor, nCOut, [nHKernel, nWKernel], trt.Weights(w_local), trt.Weights(b))
...
def case_int8qdq():
    ...
    layer = tw.network.add_convolution_nd(layer_dq0.get_output(0), nCOut, [nHKernel, nWKernel], trt.Weights(), trt.Weights(np.ascontiguousarray(b)))  # weight as empty
