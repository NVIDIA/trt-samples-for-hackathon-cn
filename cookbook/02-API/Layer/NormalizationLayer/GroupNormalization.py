...
shape = [1, 4, 3, 5]
nGroup = 2
shapeSB = [1, nGroup, 1, 1]
data0 = np.arange(np.prod(shape[2:]), dtype=np.float32).reshape(1, 1, *shape[2:])
data1 = np.arange(np.prod(shape[2:]), dtype=np.float32).reshape(1, 1, *shape[2:]) + 100
data2 = np.zeros(shape[2:], dtype=np.float32).reshape(1, 1, *shape[2:])
data3 = np.ones(shape[2:], dtype=np.float32).reshape(1, 1, *shape[2:])
...
normalizationLayer.num_groups = nGroup  # set number of groups
