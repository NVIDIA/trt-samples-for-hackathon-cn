...
data = np.arange(1, 1 + nB * nC * nW * nW, dtype=np.float32).reshape(nB, nC, nH, nW)  # 输入数据
...
#-------------------------------------------------------------------------------# 网络部分
shift = np.ascontiguousarray(np.full([nC, nH, nW], -7.0, dtype=np.float32))  # 参数元素数等于输入张量元素数
