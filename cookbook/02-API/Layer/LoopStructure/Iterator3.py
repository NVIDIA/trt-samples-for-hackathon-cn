...
limit = network.add_constant((), np.array([nW], dtype=np.int32))  # 循环次数变为 nW
loop.add_trip_limit(limit.get_output(0), trt.TripLimit.COUNT)

_H0 = network.add_constant([1, nC, nH], np.ones(nC * nH, dtype=np.float32))  # 循环体输入张量，尺寸变为 [1,nC,nW]
...
lengthLayer = network.add_constant((), np.array([nW], dtype=np.int32))  # 保存长度变为 nW
