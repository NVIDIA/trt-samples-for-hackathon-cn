...
# Remove a tensor from the network (commonly used while editing a network parsed from ONNX)
tensor_scratch = network.add_input("inputT2", trt.float32, [len(shape)])  # Add a scratch input tensor
print(f"{network.num_inputs = } (before remove_tensor)")
network.remove_tensor(tensor_scratch)  # Remove the scratch tensor, `remove_tensor` requires the tensor is not used by any layer
