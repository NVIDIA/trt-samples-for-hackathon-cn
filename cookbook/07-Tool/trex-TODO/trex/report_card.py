...
def report_card_perf_overview(plan: EnginePlan):
    ...
    def precision_per_type(title):
        ...
        [precision_colormap[p] for p in df["precision"]]
...
def report_card_gemm_MNK_scatter(plan: pd.DataFrame):
    ...
    def render_scatter(choice, x, y, color, size):
        ...
        fig.update_layout(margin=dict(l=0, r=0, b=0, t=50), title=choice, title_x=0.5)
        fig.show()

    dropdown_choices = {
        "M vs. Latency (color = foorprint)": ("attr.M", "latency.avg_time", "total_footprint_bytes", None),
        "N vs. Latency (color = foorprint)": ("attr.N", "latency.avg_time", "total_footprint_bytes", None),
        "K vs. Latency (color = foorprint)": ("attr.K", "latency.avg_time", "total_footprint_bytes", None),
    }
...
def report_card_efficiency_vs_latency_3d(plan: pd.DataFrame):
    convs = plan.get_layers_by_type("Convolution")
    fig = px.scatter_3d(convs, x="attr.compute_efficiency", y="attr.memory_efficiency", z="latency.avg_time", color="total_footprint_bytes", size="latency.avg_time", size_max=18, opacity=0.7)

    fig.update_layout(margin=dict(l=0, r=0, b=0, t=50), title="Compute-efficiency vs Memory-efficiency vs Latency", title_x=0.5)
