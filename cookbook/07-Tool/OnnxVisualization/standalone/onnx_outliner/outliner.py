...
def _tensorrt_check(onnx_file: Path) -> dict:
    ...
    The builder / network / logger triple is spelled out here rather than reused
    out of the cookbook's `TRTWrapperV1`, so that this copy of the package needs
    nothing outside `onnx` / `onnx_graphsurgeon` / `numpy`. TensorRT is
    imported lazily and its absence only skips the check: the outliner is useful
    without TensorRT installed.
    """
    try:
    ...
    try:
        ...
        return {"status": "pass", "n_layer": network.num_layers}
    except Exception as e:
        return {"status": "error", "reason": f"{type(e).__name__}: {e}"}
