...
onnx_file = f"model_{Path(__file__).name.split('.')[0]}"
onnx_file_0 = Path(onnx_file + "_00.onnx")
onnx_file_1 = Path(onnx_file + "_01.onnx")
onnx_file_2 = Path(onnx_file + "_02.onnx")

onnx_file_0.unlink(missing_ok=True)
onnx_file_1.unlink(missing_ok=True)
onnx_file_2.unlink(missing_ok=True)
Path(str(onnx_file_0) + ".weight").unlink(missing_ok=True)
Path(str(onnx_file_1) + ".weight").unlink(missing_ok=True)
Path(str(onnx_file_2) + ".weight").unlink(missing_ok=True)
