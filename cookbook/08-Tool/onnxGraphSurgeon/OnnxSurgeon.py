...
for node in graph.nodes:
    if node.op == 'Add' and node.name == 'node-1':
        ...
        batchNormVariable   = gs.Variable("MyAddPluginVariable-0", np.dtype(np.float32), None)
        batchNormNode       = gs.Node("BatchNormalization","BatchNormalization-0",
                                        inputs=[node.inputs[0]],
                                        outputs=[batchNormVariable])
        graph.nodes.append(batchNormNode)
        node.o().inputs[0] = batchNormVariable
        node.outputs.clear()
