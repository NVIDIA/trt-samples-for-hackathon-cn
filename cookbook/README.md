...
```bash
docker run     -it -e NVIDIA_VISIBLE_DEVICES=0 --gpus "device=0"     --shm-size 16G --ulimit memlock=-1 --ulimit stack=67108864     --name trt-cookbook     -v <PathToRepo>:/trtcookbook     nvcr.io/nvidia/pytorch:25.02-py3     /bin/bash
```
