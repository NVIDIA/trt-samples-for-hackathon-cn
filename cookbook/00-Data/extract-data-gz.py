...
def extract_mnist_data(gz_path, is_image):
    dt = np.dtype(np.uint32).newbyteorder(">")
    with gzip.open(gz_path, "rb") as byte_stream:
        magic = np.frombuffer(byte_stream.read(4), dtype=dt)[0]

        if is_image:
            if magic != 2051:
                raise ValueError(f"Fail reading image file: {gz_path}")
        ...
        if magic != 2049:
            raise ValueError(f"Fail reading label file: {gz_path}")
...
if __name__ == "__main__":
    ...
    train_image = extract_mnist_data(mnist_data_path / "train-images-idx3-ubyte.gz", is_image=True)
    train_label = extract_mnist_data(mnist_data_path / "train-labels-idx1-ubyte.gz", is_image=False).astype(np.float32)
    test_image = extract_mnist_data(mnist_data_path / "t10k-images-idx3-ubyte.gz", is_image=True)
    test_label = extract_mnist_data(mnist_data_path / "t10k-labels-idx1-ubyte.gz", is_image=False).astype(np.float32)
