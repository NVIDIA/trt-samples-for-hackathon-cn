...
namespace
{
inline constexpr char PLUGIN_NAME[] {"AddScalarTriton"};
...
namespace nvinfer1
{
// Identical in structure to 05-Plugin/BasicExample/AddScalarPlugin: the only difference is that
// `enqueue` calls into an ahead-of-time compiled Triton kernel instead of a `__global__` function.
class AddScalarTritonPlugin : public IPluginV3, public IPluginV3OneCore, public IPluginV3OneBuild, public IPluginV3OneRuntime
{
private:
    float                    mScalar {0.0f};
...
public:
    AddScalarTritonPlugin() = delete;
    AddScalarTritonPlugin(float const scalar);
    AddScalarTritonPlugin(AddScalarTritonPlugin const &p) = default;
...
class AddScalarTritonPluginCreator : public IPluginCreatorV3One
{
private:
