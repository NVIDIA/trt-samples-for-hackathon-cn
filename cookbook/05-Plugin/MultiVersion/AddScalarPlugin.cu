...
IPluginV3 *AddScalarPluginV2::clone() noexcept
{
    WHERE_AM_I();
    std::unique_ptr<AddScalarPluginV2> p {std::make_unique<AddScalarPluginV2>(*this)};
    p->initFieldsToSerialize();
    return p.release();
}

char const *AddScalarPluginV2::getPluginVersion() const noexcept
{
    WHERE_AM_I();
...
{
    printf("######## Print something to show the difference between V1 and V2 ########\n");
    return;
}

int32_t AddScalarPluginV2::enqueue(PluginTensorDesc const *inputDesc, PluginTensorDesc const *outputDesc, void const *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept
{
    WHERE_AM_I();
    int nElement = 1;
    for (int i = 0; i < inputDesc[0].dims.nbDims; ++i)
...
IPluginV3 *AddScalarPluginCreatorV2::createPlugin(char const *name, PluginFieldCollection const *fc, TensorRTPhase phase) noexcept
{
    WHERE_AM_I();
    float scalar {0};
    for (int32_t i = 0; i < fc->nbFields; ++i)
    ...
    return new AddScalarPluginV2(scalar);
}

char const *AddScalarPluginCreatorV2::getPluginVersion() const noexcept
{
    WHERE_AM_I();
    return PLUGIN_VERSION_V2;
}
