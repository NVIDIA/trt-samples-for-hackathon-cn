...
__global__ void addScalarInt8Kernel(int8_t const *input, int8_t *output, float const scalar, float const inputScale, float const outputScale, int const nElement)
{
    int const index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= nElement)
        return;

    float x       = float(input[index]) * inputScale;
    float y       = (x + scalar) / outputScale;
    y             = fminf(127.0f, fmaxf(-128.0f, nearbyintf(y)));
    output[index] = int8_t(y);
}
...
bool AddScalarPlugin::supportsFormatCombination(int32_t pos, DynamicPluginTensorDesc const *inOut, int32_t nbInputs, int32_t nbOutputs) noexcept
{
    ...
    {
    case 0:
        res = (inOut[0].desc.type == DataType::kFLOAT || inOut[0].desc.type == DataType::kHALF) && inOut[0].desc.format == TensorFormat::kLINEAR || inOut[0].desc.type == DataType::kINT8 && inOut[0].desc.format == TensorFormat::kCHW4;
        break;
    case 1:
...
{
    WHERE_AM_I();
    if (in[0].type == DataType::kINT8 && out[0].type == DataType::kINT8)
    {
        ...
        if (mOutputScale <= 0.0f)
        {
            mOutputScale = mInputScale;
        }
        // Some calibrator/plugin combinations may provide abnormal output scale values.
        // Fallback to input scale for this element-wise add plugin.
        if (mOutputScale > mInputScale * 1e6f || mOutputScale < mInputScale * 1e-6f)
        {
            mOutputScale = mInputScale;
        }
        printf("    [onShapeChange] inputScale=%e, outputScale=%e\n", mInputScale, mOutputScale);
    }
    return 0;
}
...
int32_t AddScalarPlugin::enqueue(PluginTensorDesc const *inputDesc, PluginTensorDesc const *outputDesc, void const *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept
{
    ...
    case int(DataType::kFLOAT):
        ...
        // Force to use the INT8 kernel in INT8 mode in this example, which this sleep is not required in actual use
    ...
    case int(DataType::kINT8):
        printf("    [enqueue]Try INT8 kernel\n");
        (addScalarInt8Kernel)<<<grid, block, 0, stream>>>(reinterpret_cast<int8_t const *>(inputs[0]), reinterpret_cast<int8_t *>(outputs[0]), mScalar, mInputScale, mOutputScale, nElement);
        break;
    default: // should NOT be here
