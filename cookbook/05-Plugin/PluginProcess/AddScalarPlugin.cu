...
int32_t AddScalarPlugin::enqueue(const PluginTensorDesc *inputDesc, const PluginTensorDesc *outputDesc, const void *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept
{
    ...
    case int(DataType::kFLOAT):
        ...
        printf("\tFP32 kernel!\n");
#endif
        std::this_thread::sleep_for(std::chrono::milliseconds(40)); // Force the plugin to use the FP16 kernel in FP16 mode, which this sleep is not required in actual use
    ...
    case int(DataType::kHALF):
        ...
        printf("\tFP16 kernel!\n");
#endif
        std::this_thread::sleep_for(std::chrono::milliseconds(20)); // Force the plugin to use the FP16 kernel in FP16 mode, which this sleep is not required in actual use
