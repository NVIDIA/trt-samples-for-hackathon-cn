...
namespace
{
inline constexpr char WRITER_PLUGIN_NAME[] {"ResourceWriter"};
inline constexpr char READER_PLUGIN_NAME[] {"ResourceReader"};
inline constexpr char PLUGIN_NAMESPACE[] {""};
inline constexpr char PLUGIN_VERSION[] {"1"};
inline constexpr char RESOURCE_KEY[] {"ResourceExample.SharedTextResource"};
} // namespace

namespace nvinfer1
{

class SharedTextResource : public IPluginResource
{
public:
    std::atomic<int32_t> mLastSeed {0};
    std::atomic<int32_t> mWriteCount {0};

    SharedTextResource()           = default;
    ~SharedTextResource() noexcept = default;

    IPluginResource *clone() noexcept override;
    int32_t          release() noexcept override;
};

class ResourceSharePlugin : public IPluginV3, public IPluginV3OneCore, public IPluginV3OneBuild, public IPluginV3OneRuntime
{
private:
    bool                     mIsWriter {false};
    int32_t                  mSeed {0};
    SharedTextResource      *mResource {nullptr};
    bool                     mHasAcquiredResource {false};
    std::vector<PluginField> mDataToSerialize;
    PluginFieldCollection    mFCToSerialize;

    bool acquireSharedResource() noexcept;
    void releaseSharedResource() noexcept;

public:
    ResourceSharePlugin() = delete;
    ResourceSharePlugin(bool const isWriter, int32_t const seed);
    ResourceSharePlugin(ResourceSharePlugin const &plugin);
    ~ResourceSharePlugin() noexcept override;
...
class ResourceWriterPluginCreator : public IPluginCreatorV3One
{
private:
...
class ResourceReaderPluginCreator : public IPluginCreatorV3One
{
private:
    PluginFieldCollection mFC;

public:
    ResourceReaderPluginCreator();
    char const                  *getPluginName() const noexcept override;
    char const                  *getPluginVersion() const noexcept override;
    PluginFieldCollection const *getFieldNames() noexcept override;
    IPluginV3                   *createPlugin(char const *name, PluginFieldCollection const *fc, TensorRTPhase phase) noexcept override;
