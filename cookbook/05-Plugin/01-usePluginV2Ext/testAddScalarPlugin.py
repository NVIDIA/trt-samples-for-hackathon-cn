...
def run(shape, scalar):
    ...
    trtFile     = 'engine-Shape['+''.join([str(i)+'-' for i in shape[:-1]])+str(shape[-1])+'].trt'
    print("Test", testCase)
    ...
    if os.path.isfile(trtFile):
        ...
        inputT0         = network.add_input('inputT0', trt.DataType.FLOAT, shape)
    ...
    bufferD = []
    for i in range(engine.num_bindings):
        bufferD.append( cuda.cuMemAllocAsync(bufferH[i].nbytes,stream)[1] )

    for i in range(nInput):
        cuda.cuMemcpyHtoDAsync(bufferD[i], np.ascontiguousarray(bufferH[i].reshape(-1)).ctypes.data, bufferH[i].nbytes, stream)

    context.execute_async_v2(bufferD, stream)

    for i in range(nOutput):
        cuda.cuMemcpyDtoHAsync(bufferH[nInput+i].ctypes.data, bufferD[nInput+i], bufferH[nInput+i].nbytes, stream)
