...
__global__ void printGPU(float *gpu, int n)
{
    for (int i = 0; i < n; ++i)
        printf("%f\n", gpu[i]);
}
...
{
    WHERE_AM_I();
    return (m_.nMaxCol > 32) ? ALIGN_TO(sizeof(int) * m_.nMaxRow * m_.nMaxCol, 1024) : 0;
}
...
int32_t MultinomialDistributionPlugin::enqueue(const PluginTensorDesc *inputDesc, const PluginTensorDesc *outputDesc, const void *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept
{
    ...
    thrust::device_vector<float>            rangeData(nRow);
    thrust::counting_iterator<unsigned int> indexBegin(0);
    thrust::transform(indexBegin, indexBegin + nRow, rangeData.begin(), RandomGeneratorCudaFunctor<float>(m_.seed));
    ...
    if (inputDesc[0].type == DataType::kHALF)
    {
        ...
        {
        case 4:
            (sampleSmallKernel<4>)<<<nRow, 32, 0, stream>>>((__half *)inputs[0], thrust::raw_pointer_cast(&rangeData[0]), (int *)outputs[0], (__half *)outputs[1]);
            break;
        case 32:
            (sampleSmallKernel<32>)<<<nRow, 32, 0, stream>>>((__half *)inputs[0], thrust::raw_pointer_cast(&rangeData[0]), (int *)outputs[0], (__half *)outputs[1]);
            break;
        case 128:
        {
            (sampleLargeKernel<128>)<<<nRow, ALIGN_TO(nCol, 32), 0, stream>>>((__half *)inputs[0], thrust::raw_pointer_cast(&rangeData[0]), (int *)outputs[0], (__half *)outputs[1], (int *)workspace);
            break;
        }
        default:
    ...
    else
    {
        ...
        {
        case 4:
            (sampleSmallKernel<4>)<<<nRow, 32, 0, stream>>>((float *)inputs[0], thrust::raw_pointer_cast(&rangeData[0]), (int *)outputs[0], (float *)outputs[1]);
            break;
        case 32:
            (sampleSmallKernel<32>)<<<nRow, 32, 0, stream>>>((float *)inputs[0], thrust::raw_pointer_cast(&rangeData[0]), (int *)outputs[0], (float *)outputs[1]);
            break;
        case 128:
        {
            (sampleLargeKernel<128>)<<<nRow, ALIGN_TO(nCol, 32), 0, stream>>>((float *)inputs[0], thrust::raw_pointer_cast(&rangeData[0]), (int *)outputs[0], (float *)outputs[1], (int *)workspace);
            break;
        }
        default:
