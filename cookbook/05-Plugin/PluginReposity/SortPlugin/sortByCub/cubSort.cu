...
#include <cstdio>
#include <algorithm>
//#include </usr/local/cub/cub/device/device_radix_sort.cuh>
...
int main()
{
    ...
    for(int i = 0;i < nItem; i++)
    {
        printf("%3d: input(%.4E,%3d), outputCPU(%.4E,%3d), outputGPU(%.4E,%3d)\n",
            i, hKey[i], hValue[i], hKeyCPU[i], hValueCPU[i], hKeyGPU[i], hValueGPU[i]);
    }

    if (hKey)               delete[] hKey;
    if (hKeyCPU)            delete[] hKeyCPU;
    if (hKeyGPU)            delete[] hKeyGPU;
    if (hValue)             delete[] hValue;
    if (hValueCPU)          delete[] hValueCPU;
    if (hValueGPU)          delete[] hValueGPU;
    if (dKey.d_buffers[0])  cudaFree(dKey.d_buffers[0]);
    if (dKey.d_buffers[1])  cudaFree(dKey.d_buffers[1]);
    if (dValue.d_buffers[0])cudaFree(dValue.d_buffers[0]);
    if (dValue.d_buffers[1])cudaFree(dValue.d_buffers[1]);
    if (dTemp)              cudaFree(dTemp);
