...
def getLayerNormPlugin(nHiddenDimension):
    for c in trt.get_plugin_registry().plugin_creator_list:
        #print(c.name)
        if c.name == 'LayerNorm':
            parameterList = []
            parameterList.append(trt.PluginField('nHiddenDimension', np.int32(nHiddenDimension), trt.PluginFieldType.INT32))
...
def run(shape, bFp16):
    ...
    if os.path.isfile(trtFile):
        ...
        pluginLayer = network.add_plugin_v2([inputT0, inputT1, inputT2], getLayerNormPlugin(shape[-1]))
