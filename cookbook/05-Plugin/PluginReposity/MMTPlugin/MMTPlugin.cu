...
#if DEBUG_ENABLE
#define DEBUG_FUNC() do {std::cout << "\tDEBUG:\t" << this << "\t" << __func__ << std::endl;} while(0)
#else
#define DEBUG_FUNC()
#endif
...
int trt_dtype_size(nvinfer1::DataType dtype) {
    if (dtype == nvinfer1::DataType::kFLOAT) return 4;
    if (dtype == nvinfer1::DataType::kHALF) return 2;
...
int cublas_dtype_size(cudaDataType_t dtype) {
    if (dtype == CUDA_R_32F) return 4;
    if (dtype == CUDA_R_16F) return 2;
    assert(0); // should NOT be here
    return 0;
}
...
cublasGemmAlgo_t find_fastest_gemm_algo(cublasHandle_t handle,
    ...
    for (int i = 0; i < num_algo; ++i) {
        ...
        auto status = row_major_gemm(handle,
            ...
            m, n, k,
            &alpha,
            reinterpret_cast<const void*>(A), cublas_dtype<T>(), lda,
            reinterpret_cast<const void*>(B), cublas_dtype<T>(), ldb,
            &beta,
            C, cublas_dtype<T>(), ldc,
...
cublasGemmAlgo_t find_fastest_batched_gemm_algo(cublasHandle_t handle,
    ...
    for (int i = 0; i < num_algo; ++i) {
        ...
        auto status = row_major_batched_gemm(handle,
            ...
            m, n, k,
            &alpha,
            const_cast<const void**>(Aarray), cublas_dtype<T>(), lda,
            const_cast<const void**>(Barray), cublas_dtype<T>(), ldb,
            &beta,
            Carray, cublas_dtype<T>(), ldc,
            batchCount,
            CUDA_R_32F,//CUBLAS_COMPUTE_32F,
            algo);
...
{
public:
    MatchMatrixTensor(const std::string& name, nvinfer1::Weights w, int h, int dim_t,
        bool deep_copy_weight=false) :
    ...
    MatchMatrixTensor(const std::string& name, const void* data, size_t length) : name_(name), own_w_(true) {
        // deserialization
        DEBUG_FUNC();
    ...
    int enqueue(const nvinfer1::PluginTensorDesc* inputDesc, const nvinfer1::PluginTensorDesc* outputDesc,
        ...
        CHECK(cudaMemcpyAsync(dev_a_array_, host_a_array_, offset*sizeof(void*),
            cudaMemcpyHostToDevice, copy_stream_));
        CHECK(cudaMemcpyAsync(dev_b_array_, host_b_array_, offset*sizeof(void*),
            cudaMemcpyHostToDevice, copy_stream_));
        CHECK(cudaMemcpyAsync(dev_c_array_, host_c_array_, offset*sizeof(void*),
            cudaMemcpyHostToDevice, copy_stream_));
...
thrust::host_vector<T> padd_input(const thrust::host_vector<T>& unpadd,
    int emb_size, const std::vector<int>& lod) {
