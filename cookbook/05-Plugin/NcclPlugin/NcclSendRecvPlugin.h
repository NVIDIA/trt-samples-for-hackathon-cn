...
namespace
{
static char const *PLUGIN_NAME {"NcclSendRecv"};
...
class NcclSendRecvPlugin : public IPluginV3, public IPluginV3OneCore, public IPluginV3OneBuild, public IPluginV3OneRuntime
{
private:
    int32_t                                  mMode {0}; // 0: send, 1: recv
    int32_t                                  mRank {0};
    int32_t                                  mPeer {1};
    int32_t                                  mWorldSize {2};
    std::array<int8_t, NCCL_UNIQUE_ID_BYTES> mUniqueId {};
...
public:
    NcclSendRecvPlugin() = delete;
    NcclSendRecvPlugin(int32_t mode, int32_t rank, int32_t peer, int32_t worldSize, std::array<int8_t, NCCL_UNIQUE_ID_BYTES> const &uniqueId);
    NcclSendRecvPlugin(NcclSendRecvPlugin const &p) = default;
...
private:
    int32_t initializeCommIfNeeded() noexcept;
};

class NcclSendRecvPluginCreator : public IPluginCreatorV3One
{
private:
