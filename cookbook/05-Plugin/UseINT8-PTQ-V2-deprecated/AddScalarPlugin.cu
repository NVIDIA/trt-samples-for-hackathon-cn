...
int32_t AddScalarPlugin::enqueue(const PluginTensorDesc *inputDesc, const PluginTensorDesc *outputDesc, const void *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept
{
    ...
    case int(DataType::kFLOAT):
        printf("    [enqueue]FP32 kernel\n");
        // Force the plugin to use the INT8 kernel in INT8 mode, which this sleep is not required in actual use
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        (addScalarKernel<float>)<<<grid, block, 0, stream>>>(reinterpret_cast<const float *>(inputs[0]), reinterpret_cast<float *>(outputs[0]), m.scalar, nElement);
        break;
    case int(DataType::kINT8):
        printf("    [enqueue]INT8 kernel\n");
        (addScalarKernel<int8_t>)<<<grid, block, 0, stream>>>(reinterpret_cast<const int8_t *>(inputs[0]), reinterpret_cast<int8_t *>(outputs[0]), int8_t(m.scalar / m.inputScale), nElement);
        break;
    default: // should NOT be here
