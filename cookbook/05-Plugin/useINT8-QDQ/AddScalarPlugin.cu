...
int32_t AddScalarPlugin::enqueue(const PluginTensorDesc *inputDesc, const PluginTensorDesc *outputDesc, const void *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept
{
    ...
    if (inputDesc[0].type == DataType::kFLOAT)
    {
        ...
        printf("\tFP32 kernel!\n");
#endif
        std::this_thread::sleep_for(std::chrono::milliseconds(40)); // 迫使范例程序在 INT8 模式下使用 INT8 的 kernel，实际使用时不需要这个等待
    ...
    else if (inputDesc[0].type == DataType::kINT8)
    {
        ...
        printf("\tINT8 kernel!\n");
#endif
        std::this_thread::sleep_for(std::chrono::milliseconds(20)); // 迫使范例程序在 INT8 模式下使用 INT8 的 kernel，实际使用时不需要这个等待
