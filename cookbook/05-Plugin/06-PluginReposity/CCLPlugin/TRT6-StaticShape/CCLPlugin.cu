...
{
    return int8 {
        idx - (nWidth + 2) - 1, idx - (nWidth + 2), idx - (nWidth + 2) + 1,
        idx - 1,                                    idx + 1,
        idx + (nWidth + 2) - 1, idx + (nWidth + 2), idx + (nWidth + 2) + 1
    };
}
...
public:
    CCL(void *workspace, int batchSize, int nHeight, const int nWidth): batchSize(batchSize), nHeight(nHeight), nWidth(nWidth)
    {
    ...
    {
        initializeLabelKernel   <<<dim3((nWidth + 2 + 31) / 32, (nHeight + 2 + 31) / 32), dim3(32, 32), 0, stream>>>
                                    ((float*)input[0], (float*)input[1], labelMap, linkMap, batchSize, nHeight, nWidth, minPixelScore, minLinkScore);
        initializeConnectKernel <<<dim3((nWidth + 31) / 32, ((nHeight + 1) / 2 + 31) / 32), dim3(32, 32), 0, stream>>>
                                    ((char8*)linkMap, batchSize, nHeight, nWidth);

        cudaMemsetAsync(whichNeedLink, 1, batchSize * sizeof(int), stream);
        for (int i = 0, needIteration = 1; needIteration; i++)
        {
            linkKernel          <<<dim3((nWidth + 31) / 32, (nHeight + 31) / 32), dim3(32, 32), 0, stream>>>
                                    (labelMap, linkMap, batchSize, nHeight, nWidth, whichNeedLink, i != 0);

            cudaMemsetAsync(temp, 0, sizeof(int) * batchSize, stream);                              // temp used as 'whichNeedLink' in the next turn

            ResolveKernel       <<<dim3((nWidth + 31) / 32, (nHeight + 31) / 32), dim3(32, 32), 0, stream>>>
                                    (labelMap, linkMap, batchSize, nHeight, nWidth, whichNeedLink, i ? temp : NULL);
    ...
    void exportResult(void **output, cudaStream_t stream)
    {
...
int LayerNormPlugin::enqueue(int batchSize, const void * const *input, void **output, void* workspace, cudaStream_t stream)
{
    CCL ccl(workspace, batchSize, m.height, m.width);
    ccl.calculate(input, m.minPixelScore, m.minLinkScore, m.minArea, stream);
    ccl.exportResult(output, stream);
