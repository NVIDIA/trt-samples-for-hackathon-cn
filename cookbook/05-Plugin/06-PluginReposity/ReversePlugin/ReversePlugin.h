...
class ReversePlugin: public IPluginV2DynamicExt
{
private:
    struct
    {
...
public:
    ...
    ReversePlugin(const void *buffer, size_t length) 
    {
    ...
    {
        return new ReversePlugin(&m, sizeof(m));
    }
    ...
    bool supportsFormatCombination(int pos, const PluginTensorDesc* inOut, int nbInputs, int nbOutputs) override
    {
    	...
    	case 1:
    		return inOut[1].format == TensorFormat::kLINEAR && inOut[1].type == DataType::kINT32;
        case 2:
            return inOut[2].format == TensorFormat::kLINEAR && inOut[2].type == inOut[0].type;
        default:
        return false;
        }
    }
    ...
    const char* getPluginType() const override {return "ReversePlugin";}
...
class ReversePluginCreator : public IPluginCreator
{
public:
    ...
    const char* getPluginNamespace() const override {return "";}    
    const char* getPluginName() const override {return "ReversePlugin";}    
    const char* getPluginVersion() const override {return "0";}
    void setPluginNamespace(const char* szNamespace) override {}    
    const PluginFieldCollection* getFieldNames() override{return nullptr;}
    IPluginV2* deserializePlugin(const char* name, const void* serialData, size_t serialLength) override {return new ReversePlugin(serialData, serialLength);}
};
