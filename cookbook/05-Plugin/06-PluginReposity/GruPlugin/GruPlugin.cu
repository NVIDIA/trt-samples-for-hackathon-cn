...
template <>
__device__ __forceinline__ float fp_convert<float, float>(float in) { return in; };
template <>
__device__ __forceinline__ float fp_convert<__half, float>(__half in) { return __half2float(in); };
template <>
__device__ __forceinline__ __half fp_convert<float, __half>(float in) { return __float2half(in); };
template <>
__device__ __forceinline__ __half fp_convert<__half, __half>(__half in) { return in; };
...
GruPlugin::GruPlugin(const std::string name, const void * data, size_t length) : mLayerName(name)
{
...
inline int32_t GruPlugin::enqueue(const PluginTensorDesc *inputDesc, const PluginTensorDesc *outputDesc, const void *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream)
{
    ...
    if(dataType == DataType::kFLOAT)
    {
        ...
        status = callGruKernel<float>(stream, mCuBlasHandle, batchSize, maxSeqLen, mInputSize, mHiddenSize,
                                        mWeightsX_d, mWeightsH_d, mBias_d, preStateCell_d, matMulRes_d, rhCell_d,
                                        (float *)inputs[0], (int *)inputs[1], (float *)outputs[0], (float *)outputs[1]);
    }
    else
    {
