...
public:    
    ...
    WherePlugin() {}    
    WherePlugin(const void *buffer, size_t length) {memcpy(&m, buffer, sizeof(m));}

    IPluginV2Ext* clone() const override {return new WherePlugin(&m, sizeof(m));}
    ...
    const char* getPluginType() const override { return "WherePlugin";}
...
public:        
    ...
    IPluginV2* deserializePlugin(const char* name, const void* serialData, size_t serialLength) override {return new WherePlugin(serialData, serialLength);}
    const char* getPluginName() const override {return "WherePlugin";}
    const char* getPluginVersion() const override {return "0";}
    void setPluginNamespace(const char* szNamespace) override {}
    const char* getPluginNamespace() const override {return "";}
