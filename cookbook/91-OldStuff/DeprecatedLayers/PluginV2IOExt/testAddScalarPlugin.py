...
def get_add_scalar_plugin(scalar):
    # Deprecated interface: IPluginV2IOExt plugins are loaded through the removed add_plugin_v2 API; use IPluginV3 + add_plugin_v3 instead.
...
def run(shape, scalar):
    # This PluginV2IOExt example relies on implicit-batch mode, which TRTWrapperV1 (explicit batch) cannot express, so minimal manual boilerplate is kept.
