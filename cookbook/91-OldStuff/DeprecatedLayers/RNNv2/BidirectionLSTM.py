...
def case_simple():
    ...
    data = {
        ...
        "hidden_state": np.zeros([n_c, 2, n_hidden], dtype=np.float32),  # Initial hidden state, 2 directions
        "cell_state": np.zeros([n_c, 2, n_hidden], dtype=np.float32),  # Initial cell state, 2 directions
    }
    ...
    tensor1 = tw.network.add_input("hidden_state", trt.float32, [n_c, 2, n_hidden])
    tensor2 = tw.network.add_input("cell_state", trt.float32, [n_c, 2, n_hidden])
    ...
    layer.direction = trt.RNNDirection.BIDIRECTION  # Forward and backward passes occupy layer index 0 and 1
    layer.hidden_state = tensor1
    layer.cell_state = tensor2
    for direction in range(2):
        for kind in [trt.RNNGateType.INPUT, trt.RNNGateType.CELL, trt.RNNGateType.FORGET, trt.RNNGateType.OUTPUT]:
            layer.set_weights_for_gate(direction, kind, True, trt.Weights(weight_x))
            layer.set_weights_for_gate(direction, kind, False, trt.Weights(weight_h))
            layer.set_bias_for_gate(direction, kind, True, trt.Weights(bias_x))
            layer.set_bias_for_gate(direction, kind, False, trt.Weights(bias_h))
...
if __name__ == "__main__":
    # A case of a bidirectional LSTM RNNv2 layer
    case_simple()
