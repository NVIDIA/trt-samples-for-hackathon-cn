...
def case_simple():
    ...
    # 1-layer ReLU RNN, hidden width n_hidden, sequence length n_h, embedding width n_w
    layer = tw.network.add_rnn_v2(tensor, 1, n_hidden, n_h, trt.RNNOperation.RELU)
    layer.set_weights_for_gate(0, trt.RNNGateType.INPUT, True, trt.Weights(weight_x))  # X transform, shape (n_hidden, n_w)
    layer.set_weights_for_gate(0, trt.RNNGateType.INPUT, False, trt.Weights(weight_h))  # H transform, shape (n_hidden, n_hidden)
    layer.set_bias_for_gate(0, trt.RNNGateType.INPUT, True, trt.Weights(bias_x))  # X bias, shape (n_hidden,)
    layer.set_bias_for_gate(0, trt.RNNGateType.INPUT, False, trt.Weights(bias_h))  # H bias, shape (n_hidden,)
    # Output 0: full output sequence, Output 1: final hidden state
...
if __name__ == "__main__":
    # A simple case of using the deprecated RNNv2 layer
    case_simple()
