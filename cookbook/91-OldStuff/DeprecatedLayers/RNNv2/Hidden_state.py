...
def case_simple():
    ...
    h0_shape = [n_c, 1, n_hidden]
    layer_h0 = tw.network.add_constant(h0_shape, trt.Weights(np.ascontiguousarray(np.ones(h0_shape, dtype=np.float32))))  # Initial hidden state
    ...
    layer.hidden_state = layer_h0.get_output(0)  # Set the initial hidden state, default: all zeros
...
if __name__ == "__main__":
    # A case of setting a non-zero initial hidden state for the RNNv2 layer
    case_simple()
