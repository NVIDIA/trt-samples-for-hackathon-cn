...
@case_mark
def case_simple(shape):
    trt_file = Path(f"model-{shape[2]}.plan")
    input_data = {"inputT0": np.random.rand(np.prod(shape)).astype(np.float32).reshape(shape)}

    tw = TRTWrapperV1(trt_file=trt_file, plugin_file_list=[so_file])
    if tw.engine_bytes is None:  # Create engine from scratch
        input_tensor = tw.network.add_input("inputT0", trt.float32, [-1 for _ in shape])
