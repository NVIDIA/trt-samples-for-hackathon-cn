...
def case_simple():
    scalar = 1.0
    shape = [1, 8, 2, 3]  # CHW4 format needs an input tensor with at least 4 dimensions
    ...
    if tw.engine_bytes is None:  # Create engine from scratch
        ...
        # tw.builder_config.int8_calibrator = CookbookCalibratorV1(1, shape, int8_cache_file)  # Bug in Int8 Calibrator, use dynamic range instead
        ...
        tensor.dynamic_range = [-md - abs(scalar), md + abs(scalar)]

        # Insert a cast layer to get float32 output, or int8 directly from the plugin layer
