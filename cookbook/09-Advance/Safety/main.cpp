...
void run()
{
    ...
    else
    {
        ...
        ITensor *       inputTensor   = network->addInput("inputT0", DataType::kFLOAT, Dims32 {3, {3, 4, 5}}); // safety 模式仅支持 explicit batch 模式 + static shape 模式
    ...
    for (int i = 0; i < nBinding; ++i)
    {
        ...
        std::cout << shapeToString(engine->getBindingDimensions(i)) << std::string(" "); // safe context 没有 getBindingDimensions 方法
