...
void run()
{
    ...
    for (int i = 0; i < nBinding; ++i)
    {
        CHECK(cudaHostAlloc(&vBufferH[i], vBindingSize[i], cudaHostAllocWriteCombined));
        CHECK(cudaMallocAsync(&vBufferD[i], vBindingSize[i], stream));

        float *pData = (float *)vBufferH[i];
        for (int j = 0; j < vBindingSize[i] / dataTypeToSize(engine->getBindingDataType(i)); ++j)
        {
            pData[j] = float(j);
        }
    }
