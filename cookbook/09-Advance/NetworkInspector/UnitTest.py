...
if "profile" not in locals().keys():
    extractModel(builder, config, network)
else:
    extractModel(builder, config, network, [profile])  # seems ugly if we can not get optimization profile from BuilderConfig

engineString = rebuildModel(logger)

engine = trt.Runtime(logger).deserialize_cuda_engine(engineString)
context = engine.create_execution_context()
context.set_binding_shape(0, data.shape)
#context.set_shape_input(1, data1)
