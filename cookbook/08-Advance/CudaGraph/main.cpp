...
void run()
{
    ...
    for (auto const name : tensorNameList)
    {
        if (engine->getTensorIOMode(name) == TensorIOMode::kINPUT)
        {
            ...
            CHECK(cudaMemcpyAsync(deviceBuffer, hostBuffer, nByte, cudaMemcpyHostToDevice, stream));
        }
    }
    ...
    for (auto const name : tensorNameList)
    {
        if (engine->getTensorIOMode(name) == TensorIOMode::kOUTPUT)
        {
            ...
            void *deviceBuffer = std::get<1>(bufferMap[name]);
            int   nByte        = std::get<2>(bufferMap[name]);
            CHECK(cudaMemcpyAsync(hostBuffer, deviceBuffer, nByte, cudaMemcpyDeviceToHost, stream));
        }
    }

    //cudaStreamSynchronize(stream); // Do not synchronize during capture
