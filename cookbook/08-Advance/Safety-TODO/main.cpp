...
void run()
{
    ...
    else
    {
        ...
        ITensor        *inputTensor   = network->addInput("inputT0", DataType::kFLOAT, Dims64 {3, {3, 4, 5}}); // only Explicit Batch + Static Shape is supported in safety mode
    ...
    for (int i = 0; i < nBinding; ++i)
    {
        Dims64 dim  = engine->getBindingDimensions(i);
        int    size = 1;
    ...
    for (int i = 0; i < nBinding; ++i)
    {
        printArrayInformation((float *)vBufferH[i], std::string(engine->getBindingName(i)), engine->getBindingDimensions(i), true, true);
    }
