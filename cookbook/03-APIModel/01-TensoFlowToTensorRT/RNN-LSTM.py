...
def smallTest():
    np.random.seed(97)
    nBatchSize,nSequenceLength,nInputDim    = 1,1,7
    nHiddenDim                              = 5

    def sigmoid(x):
        return 1 / ( 1 + np.exp(-x) )

    x  = np.random.rand(nBatchSize,nSequenceLength,nInputDim).astype(np.float32).reshape([nBatchSize,nSequenceLength,nInputDim])
    h0 = np.random.rand(nBatchSize,nHiddenDim).astype(np.float32).reshape([nBatchSize,nHiddenDim])
    c0 = np.random.rand(nBatchSize,nHiddenDim).astype(np.float32).reshape([nBatchSize,nHiddenDim])
    ...
    it = sigmoid( np.matmul(x,weight[0][0]) + np.matmul(h0,weight[0][1]) + bias[0] )
    ct_= np.tanh( np.matmul(x,weight[1][0]) + np.matmul(h0,weight[1][1]) + bias[1] )
    ft = sigmoid( np.matmul(x,weight[2][0]) + np.matmul(h0,weight[2][1]) + bias[2] )
    ot = sigmoid( np.matmul(x,weight[3][0]) + np.matmul(h0,weight[3][1]) + bias[3] )
    ct = ft* c0 + it * ct_
    ht = ot * np.tanh(ct)
    print(ht)
    print(ct)
...
def test1():
    nBatchSize,nSequenceLength,nInputDim    = 2,4,7
    nHiddenDim                              = 5
    ...
    if True:
        ...
        loopOutput0 = loop.add_loop_output(hiddenStateLayer.get_output(0), trt.LoopOutput.LAST_VALUE, 0)    # 形状 (nBatchSize,nHiddenSize)，nBatchSize 个独立输出，每个输出 1 个最终隐藏状态，每个隐藏状态 nHiddenSize 维坐标
        ...
        loopOutput2 = loop.add_loop_output(cellStateLayer.get_output(0), trt.LoopOutput.LAST_VALUE, 0)      # 形状 (nBatchSize,nHiddenSize)，nBatchSize 个独立输出，每个输出 1 个最终隐藏状态，每个隐藏状态 nHiddenSize 维坐标
        #-----------------------------------------------------------------------------------------------
...
def test2():
    ...
    para                = np.load('test2.npz')                             # 注意权重形状和次序与 keras.layers.LSTM 不同
...
def test3():
    nBatchSize,nSequenceLength,nInputDim    = 2,4,7
    nHiddenDim                              = 5
    forgetBias                              = 1.0 # value used in BasicLSTMCell
    inputX  = np.random.rand(nBatchSize,nSequenceLength,nInputDim).astype(np.float32).reshape([nBatchSize,nSequenceLength,nInputDim])
    inputH  = np.random.rand(nBatchSize,nHiddenDim).astype(np.float32).reshape([nBatchSize,nHiddenDim])
    inputC  = np.random.rand(nBatchSize,nHiddenDim).astype(np.float32).reshape([nBatchSize,nHiddenDim])
    ...
    gateI = gate(network, iteratorLayer.get_output(0), weightXLayerList[0].get_output(0), hiddenStateLayer.get_output(0), weightHLayerList[0].get_output(0), biasLayerList[0].get_output(0), 'I')  # 权重顺序时 ICFO 而不是 IFCO
...
def test5():
    ...
    nHiddenDim                              = 5
    inputX  = np.random.rand(nBatchSize,nSequenceLength,nInputDim).astype(np.float32).reshape([nBatchSize,nSequenceLength,nInputDim])
    inputH  = np.random.rand(nBatchSize,nHiddenDim).astype(np.float32).reshape([nBatchSize,1,nHiddenDim])
    inputC  = np.random.rand(nBatchSize,nHiddenDim).astype(np.float32).reshape([nBatchSize,1,nHiddenDim])
    ...
    y,hc    = lstm(x,initial_state=(h0,c0), # (h0,c0) 而不是 [c0,h0]
