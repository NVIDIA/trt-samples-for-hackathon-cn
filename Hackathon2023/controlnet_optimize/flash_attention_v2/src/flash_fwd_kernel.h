...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    //unsigned long long seed = 0;//std::get<0>(seeds);
    //unsigned long long offset = (bidb * params.h + bidh) * 32 + tidx % 32;//std::get<1>(seeds) + (bidb * params.h + bidh) * 32 + tidx % 32;

    //// Save seed and offset for backward.
    //if (block_id == 0 && tidx == 0) {
    //    params.rng_state[0] = seed;
    //    params.rng_state[1] = seed;//std::get<1>(seeds);
    //}
