...
class DDIM():
    def __init__(self, *args, **kwargs):
        ...
        self.sqrt_one_minus_alphas_cumprod = torch.from_numpy(np.sqrt(1. - alphas_cumprod).astype(np.float32))
        self.log_one_minus_alphas_cumprod = torch.from_numpy(np.log(1. - alphas_cumprod).astype(np.float32))
        self.sqrt_recip_alphas_cumprod = torch.from_numpy(np.sqrt(1. / alphas_cumprod).astype(np.float32))
        self.sqrt_recipm1_alphas_cumprod = torch.from_numpy(np.sqrt(1. / alphas_cumprod - 1).astype(np.float32))
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        ...
        self.ddim_sqrt_one_minus_alphas = torch.sqrt(1. - ddim_alphas).to("cuda")
