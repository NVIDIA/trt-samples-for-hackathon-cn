        ...
        ***
        ...
        if schedule == 'discrete':
            ...
            self.cosine_t_max = math.atan(
                self.cosine_beta_max * (1. + self.cosine_s) /
                math.pi) * 2. * (1. + self.cosine_s) / math.pi - self.cosine_s
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if self.schedule == 'discrete':
            return interpolate_fn(t.reshape((-1, 1)),
                                  ...
                                  self.log_alpha_array.to(t.device)).reshape(
                                      (-1))
    ...
    def inverse_lambda(self, lamb):
        ...
        if self.schedule == 'linear':
            ...
            log_alpha = -0.5 * torch.logaddexp(-2. * lamb,
                                               torch.zeros((1, )).to(lamb))
            t_fn = lambda log_alpha_t: torch.arccos(
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                ...
                noise_uncond, noise = noise_pred_fn(x_in, t_in,
                                                    cond=c_in).chunk(2)
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        if self.thresholding:
            ...
            s = torch.quantile(torch.abs(x0).reshape((x0.shape[0], -1)),
                               p,
                               dim=1)
            s = expand_dims(
                torch.maximum(s,
    ...
    def dpm_solver_first_update(self,
        ...
        if self.predict_x0:
            ...
            x_t = (expand_dims(sigma_t / sigma_s, dims) * x -
    ...
    def singlestep_dpm_solver_second_update(self,
        ...
        if self.predict_x0:
            ...
            if model_s is None:
                model_s = self.model_fn(x, s)
            x_s1 = (expand_dims(sigma_s1 / sigma_s, dims) * x -
                    expand_dims(alpha_s1 * phi_11, dims) * model_s)
            model_s1 = self.model_fn(x_s1, s1)
            if solver_type == 'dpm_solver':
                x_t = (expand_dims(sigma_t / sigma_s, dims) * x -
                       expand_dims(alpha_t * phi_1, dims) * model_s -
                       (0.5 / r1) * expand_dims(alpha_t * phi_1, dims) *
                       (model_s1 - model_s))
            ...
            if solver_type == 'dpm_solver':
                x_t = (
                    expand_dims(torch.exp(log_alpha_t - log_alpha_s), dims) * x
                    - expand_dims(sigma_t * phi_1, dims) * model_s -
                    (0.5 / r1) * expand_dims(sigma_t * phi_1, dims) *
                    (model_s1 - model_s))
            elif solver_type == 'taylor':
                x_t = (
                    expand_dims(torch.exp(log_alpha_t - log_alpha_s), dims) * x
        ...
        if self.predict_x0:
            ...
            if model_s1 is None:
                x_s1 = (expand_dims(sigma_s1 / sigma_s, dims) * x -
                        expand_dims(alpha_s1 * phi_11, dims) * model_s)
                model_s1 = self.model_fn(x_s1, s1)
            x_s2 = (expand_dims(sigma_s2 / sigma_s, dims) * x -
                    expand_dims(alpha_s2 * phi_12, dims) * model_s +
                    r2 / r1 * expand_dims(alpha_s2 * phi_22, dims) *
                    (model_s1 - model_s))
            model_s2 = self.model_fn(x_s2, s2)
            if solver_type == 'dpm_solver':
                x_t = (expand_dims(sigma_t / sigma_s, dims) * x -
                       expand_dims(alpha_t * phi_1, dims) * model_s +
                       (1. / r2) * expand_dims(alpha_t * phi_2, dims) *
                       (model_s2 - model_s))
            elif solver_type == 'taylor':
            ...
            if model_s1 is None:
                x_s1 = (
                    ...
                    x - expand_dims(sigma_s1 * phi_11, dims) * model_s)
                model_s1 = self.model_fn(x_s1, s1)
            x_s2 = (
                expand_dims(torch.exp(log_alpha_s2 - log_alpha_s), dims) * x -
                expand_dims(sigma_s2 * phi_12, dims) * model_s -
                r2 / r1 * expand_dims(sigma_s2 * phi_22, dims) *
                (model_s1 - model_s))
            model_s2 = self.model_fn(x_s2, s2)
            if solver_type == 'dpm_solver':
                x_t = (
                    expand_dims(torch.exp(log_alpha_t - log_alpha_s), dims) * x
                    - expand_dims(sigma_t * phi_1, dims) * model_s -
                    (1. / r2) * expand_dims(sigma_t * phi_2, dims) *
                    (model_s2 - model_s))
            elif solver_type == 'taylor':
                D1_0 = (1. / r1) * (model_s1 - model_s)
                D1_1 = (1. / r2) * (model_s2 - model_s)
    ...
    def multistep_dpm_solver_second_update(self,
        ...
        if self.predict_x0:
            if solver_type == 'dpm_solver':
                x_t = (expand_dims(sigma_t / sigma_prev_0, dims) * x -
                       expand_dims(alpha_t *
            ...
            if solver_type == 'dpm_solver':
                x_t = (expand_dims(torch.exp(log_alpha_t - log_alpha_prev_0),
                                   dims) * x -
                       expand_dims(sigma_t *
                                   (torch.exp(h) - 1.), dims) * model_prev_0 -
    ...
    def multistep_dpm_solver_update(self,
        ...
        """
        if order == 1:
            return self.dpm_solver_first_update(x,
                                                t_prev_list[-1],
                                                t,
    ...
    def sample(
        ...
        if method == 'adaptive':
            ...
            if method == 'singlestep':
                ...
                timesteps_outer = self.get_time_steps(skip_type=skip_type,
                                                      t_T=t_T,
                                                      t_0=t_0,
                                                      N=K,
                                                      device=device)
            for i, order in enumerate(orders):
                t_T_inner, t_0_inner = timesteps_outer[i], timesteps_outer[i +
                                                                           1]
                timesteps_inner = self.get_time_steps(skip_type=skip_type,
                                                      t_T=t_T_inner.item(),
                                                      t_0=t_0_inner.item(),
                                                      N=order,
                                                      device=device)
                ...
                r2 = None if order <= 2 else (lambda_inner[2] -
                                              lambda_inner[0]) / h
                x = self.singlestep_dpm_solver_update(x,
                                                      vec_s,
                                                      vec_t,
                                                      order,
                                                      solver_type=solver_type,
...
def interpolate_fn(x, xp, yp):
    ...
    end_idx = torch.where(torch.eq(start_idx, cand_start_idx), start_idx + 2,
                          start_idx + 1)
