...
def srmd_degradation(x, k, sf=3):
    ...
    '''
    x = ndimage.filters.convolve(x, np.expand_dims(k, axis=2),
                                 mode='wrap')  # 'nearest' | 'mirror'
...
def add_blur(img, sf=4):
    ...
    if random.random() < 0.5:
        ...
        k = anisotropic_Gaussian(ksize=2 * random.randint(2, 11) + 3,
        ...
        if i == 0:
            ...
            if random.random() < 0.75:
                ...
                image = cv2.resize(image, (int(
                    1 / sf1 * image.shape[1]), int(1 / sf1 * image.shape[0])),
                                   interpolation=random.choice([1, 2, 3]))
            else:
                k = fspecial('gaussian', 25, random.uniform(0.1, 0.6 * sf))
                k_shifted = shift_pixel(k, sf)
                ...
                image = ndimage.filters.convolve(image,
                                                 np.expand_dims(k_shifted,
                                                                axis=2),
                                                 mode='mirror')
            ...
            # downsample3
            image = cv2.resize(image, (int(1 / sf * a), int(1 / sf * b)),
                               interpolation=random.choice([1, 2, 3]))
            image = np.clip(image, 0.0, 1.0)
...
def degradation_bsrgan_plus(img,
    ...
    if random.random() < shuffle_prob:
        ...
        shuffle_order[2:6] = random.sample(shuffle_order[2:6], len(range(2,
                                                                         6)))
        shuffle_order[9:13] = random.sample(shuffle_order[9:13],
                                            len(range(9, 13)))
...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        img_lq_bicubic = albumentations.SmallestMaxSize(
            max_size=h, interpolation=cv2.INTER_CUBIC)(image=img)["image"]
        print(img_lq.shape)
        ...
        img_concat = np.concatenate(
            [lq_bicubic_nearest, lq_nearest,
             util.single2uint(img_hq)], axis=1)
