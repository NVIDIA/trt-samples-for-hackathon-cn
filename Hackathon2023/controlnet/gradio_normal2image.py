...
model.load_state_dict(
    load_state_dict('./models/control_sd15_normal.pth', location='cuda'))
model = model.cuda()
...
def process(input_image, prompt, a_prompt, n_prompt, num_samples,
            ...
            strength, scale, seed, eta, bg_threshold):
    with torch.no_grad():
        ...
        with gr.Column():
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                detect_resolution = gr.Slider(label="Normal Resolution",
                                              ...
                                              value=384,
                                              step=1)
                bg_threshold = gr.Slider(label="Normal background threshold",
                                         minimum=0.0,
                                         maximum=1.0,
                                         value=0.4,
                                         step=0.01)
