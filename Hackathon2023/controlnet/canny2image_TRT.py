...
class hackathon():
    ...
    def load_all_engines(self, trt_engine_file_list):
        ...
        for engine_file_name in trt_engine_file_list:
            with open(engine_file_name, "rb") as f:
                engine_str = f.read()
            engine = trt.Runtime(self.trt_logger).deserialize_cuda_engine(engine_str)
    ...
    def control_outputs(self):
        ...
        control_outputs = []
        for i in range(13):
            temp = torch.randn(inter_data_shapes[i], dtype=torch.float16).cuda().contiguous()
    ...
    def initialize(self):
        ...
        engine_name_list = ["sd_control_fp16.trt",
                            "sd_encoder_fp16.trt",
                            "sd_decoder_fp16.trt",
                            "hint_fp16.trt",
                            "clip_fp16.trt",
        ...
        self.hint_out = torch.randn((batch_size, 320, 32, 48), dtype=torch.float16).cuda().contiguous()

        #---------- load clip engine ------------------------------
        self.clip_context = engine_table["clip_fp16"][1]
        self.clip_out = torch.randn((batch_size*2, 77, 768), dtype=torch.float16).cuda().contiguous()


        #---------- load sd_control engine -----------------------------
        self.sd_control_context = engine_table["sd_control_fp16"][1]
        self.img = torch.randn((batch_size, 4, 32, 48), device="cuda").to(torch.float16).contiguous()
        self.step = torch.ones((batch_size,), dtype=torch.int32).cuda().contiguous()
        ...
        self.middle_h = torch.randn((2, 1280, 4, 6), dtype=torch.float16).cuda().contiguous()
        self.emb_outputs = torch.randn((1, 1280), dtype=torch.float16).cuda().contiguous()
           
        #--------- load sd_decoder engine -------------------------------
        self.sd_decoder_context = engine_table["sd_decoder_fp16"][1]
        self.index = torch.ones((batch_size,), dtype=torch.int32).cuda().contiguous()
        self.latent = torch.zeros(2, 4, 32, 48, dtype=torch.float16).cuda().contiguous()
        
        #---------- load vae engine -------------------------------
        self.vae_context = engine_table["decoder_fp16"][1]
        self.vae_out = torch.zeros((batch_size, 256, 384, 3), dtype=torch.uint8).cuda().contiguous()

        _, self.stream_0 = cudart.cudaStreamCreate()
        _, self.stream_1 = cudart.cudaStreamCreate()
        _, self.event = cudart.cudaEventCreateWithFlags(cudart.cudaEventDisableTiming)
        self.sd_control_cudagraph = infer_cudagraph.cudagraph_engine(
            engine_table["sd_control_fp16"][0],
            engine_table["sd_control_fp16"][1],
            [self.img, self.hint_out, self.step, self.clip_out]+self.control_outputs_list,
            self.stream_0)
        
        self.sd_encoder_cudagraph = infer_cudagraph.cudagraph_engine(
            engine_table["sd_encoder_fp16"][0],
            engine_table["sd_encoder_fp16"][1],
            [self.img, self.step, self.clip_out]+[self.emb_outputs] + self.encoder_outputs+[self.middle_h],
            self.stream_1)
        
        self.sd_decoder_cudagraph = infer_cudagraph.cudagraph_engine(
            engine_table["sd_decoder_fp16"][0],
            engine_table["sd_decoder_fp16"][1],
            [self.clip_out]+ self.control_outputs_list + self.encoder_outputs + [self.middle_h, self.emb_outputs, self.latent],
            self.stream_0)

        self.vae_cudagraph = infer_cudagraph.cudagraph_engine(engine_table["decoder_fp16"][0], self.vae_context, [self.img, self.vae_out], self.stream_0)
    ...
    def ddim_sampling(self, batch_size):
        ...
        img = torch.randn((batch_size, 4, 32, 48), device="cuda").to(torch.float16)
        self.img.copy_(img)
        for i, step in enumerate(self.time_range):
            ...
            cudart.cudaStreamWaitEvent(self.stream_0, self.event, cudart.cudaEventWaitDefault)
            self.sd_decoder_cudagraph.infer()
            cudart.cudaStreamSynchronize(self.stream_0)
            nvtx.end_range(rng)
            x = DDIM_scheduler.step(self.img, self.latent, index, self.scheduler.ddim_alphas, self.scheduler.ddim_alphas_prev, self.scheduler.ddim_sqrt_one_minus_alphas)
            self.img.copy_(x)
        return self.img
    ...
    def inference_clip(self, prompts):
        tokens = self.tokenizer(prompts, truncation=True, max_length=77,
    ...
    def process(self, input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, low_threshold, high_threshold):
        with torch.no_grad():
            ...
            self.inference_clip([prompt + ', ' + a_prompt, n_prompt] * num_samples + [n_prompt] * num_samples)
