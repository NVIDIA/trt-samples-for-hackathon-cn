# 关于复赛
恭喜各位同学经过激烈角逐，顺利进入复赛！  
对于参赛者，初赛的目的是通过优化同一个模型，让大家互相交流，了解TensorRT的一般用法，积累经验。复赛的目的是让同学们把自己感兴趣的、认为有价值的、有一定难度的模型找出来，在英伟达专家的支持下把它优化好。  
...
- 代码：从原始模型出发，直到运行优化好的TensorRT模型，全过程所需的所有脚本及代码，并且需要选手在公共的代码托管平台（建议用github）将其开源出来。特别地，要求能在选手自选的某个英伟达官方Docker镜像（在[NGC](https://catalog.ngc.nvidia.com/containers)里面选择）中运行。
  ...
  - NGC里有多种镜像可选，有的预装了TensorFlow，PyTorch或TensorRT。选手只能从中选一个，然后自行安装额外所需的软件并附上 requirement.txt。
...
|6月27日|最终审查：完成代码和报告。请导师按文档审核并运行代码|等待比赛结果|
|7月6日|公布比赛结果|上网吐槽|
...
- 在Docker里面代码编译、运行步骤的完整说明
  - 最好逐行运行你给的命令就能把代码跑起来，比如从docker pull开始

## 原始模型
### 模型简介
请介绍模型的基本信息，可以包含但不限于以下内容：
- 用途以及效果
- 业界实际运用情况，比如哪些厂商、产品在用
- 整体结构，以及关键的计算结构
...
## 优化过程
这一部分是报告的主体。请把自己假定为老师，为TensorRT的初学者讲述如何从原始模型经过一系列开发步骤得到优化后的TensorRT模型。建议：
- 分步骤讲清楚开发过程
...
- 精度：报告与原始模型进行精度对比测试的结果，验证精度达标。
  - 这里的精度测试指的是针对原始模型和TensorRT优化模型分别输出的数据(tensor)做数值比较。请给出绝对误差和相对误差的统计结果。
  - 使用训练好的权重和有意义的输入数据更有说服力。如果选手使用了随机权重和输入数据，请在这里注明。  
  - 在精度损失较大的情况下，鼓励大家用训练好的权重和测试数据集对模型优化前与优化后的准确度指标做全面比较，以增强说服力
